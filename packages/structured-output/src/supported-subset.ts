export const SUPPORTED_JSON_SCHEMA_KEYWORDS = new Set([
  ...
  'format', 'oneOf', 'anyOf', 'description', 'default', '$ref', '$defs', 'nullable'
]);

export const UNSUPPORTED_KEYWORDS = new Set([
  '$id', '$schema', '$comment', 'if', 'then', 'else', 'allOf', 'not', 'patternProperties',
  'dependentRequired', 'dependentSchemas', 'unevaluatedProperties', 'unevaluatedItems',
  'contains', 'prefixItems', 'minContains', 'maxContains', 'propertyNames', 'minProperties',
  'maxProperties', 'exclusiveMinimum', 'exclusiveMaximum', 'multipleOf', 'uniqueItems',
  'contentEncoding', 'contentMediaType', 'contentSchema'
]);

export class UnsupportedSchemaKeywordError extends Error {
  constructor(public keyword: string, public path: string) {
    super("Unsupported schema keyword '" + keyword + "' at " + path);
    this.name = 'UnsupportedSchemaKeywordError';
  }
}

export function validateSupportedSubset(schema: any, strict: boolean = true, path: string = ''): string[] {
  ...
  for (const key of Object.keys(schema)) {
    ...
    if (typeof schema[key] === 'object' && schema[key] !== null && key !== 'enum' && key !== 'const') {
      warnings.push(...validateSupportedSubset(schema[key], strict, currentPath));
    }
  }
  return warnings;
}
