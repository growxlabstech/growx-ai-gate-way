...
export class ToolLoopDetector {
  ...
  constructor(
    private readonly config: LoopDetectorConfig = DEFAULT_LOOP_DETECTOR_CONFIG,
  ) {}
  ...
  incrementRound(): void {
    ...
    if (this.roundCount > this.config.maxRounds) {
      throw new ToolLoopDetectedError(
        `Tool execution exceeded maximum round limit of ${this.config.maxRounds}`,
      );
    }
  }
  ...
  recordCall(name: string, argumentsHash: string): void {
    ...
    if (this.totalCalls > this.config.maxTotalCalls) {
      throw new ToolLoopDetectedError(
        `Total tool call count ${this.totalCalls} exceeded maximum limit of ${this.config.maxTotalCalls}`,
      );
    }

    // Check consecutive identical calls
    const recent = this.callHistory.slice(
      -this.config.maxConsecutiveIdenticalCalls + 1,
    );
    const allMatch =
    ...
    if (allMatch && recent.length > 0) {
      throw new ToolLoopDetectedError(
        `Tool '${name}' with identical arguments was called ${this.config.maxConsecutiveIdenticalCalls} consecutive times. Infinite loop detected.`,
      );
    }
