...
export interface IProviderRepository {
  ...
  updateProvider(id: string, updates: Partial<ProviderEntity>): Promise<ProviderEntity>;
  listProviders(filter?: ProviderListFilter): Promise<ProviderEntity[]>;

  // Credential operations (Phase 5 compatibility)
  createCredential(credential: ProviderCredentialEntity): Promise<ProviderCredentialEntity>;
  ...
  listCredentialsByProviderId(providerId: string): Promise<ProviderCredentialEntity[]>;
  updateCredential(id: string, updates: Partial<ProviderCredentialEntity>): Promise<ProviderCredentialEntity>;
  ...
  updateAccount(id: string, updates: Partial<ProviderAccount>): Promise<ProviderAccount>;
  listAccountsByProviderId(providerId: string): Promise<ProviderAccount[]>;

  // Phase 28: Provider Credential V2 & Version operations
  createCredentialV2(credential: ProviderCredential): Promise<ProviderCredential>;
  createCredentialVersion(version: ProviderCredentialVersion): Promise<ProviderCredentialVersion>;
  getCredentialVersionById(id: string): Promise<ProviderCredentialVersion | null>;
  getActiveCredentialVersion(credentialId: string): Promise<ProviderCredentialVersion | null>;
  updateCredentialVersion(id: string, updates: Partial<ProviderCredentialVersion>): Promise<ProviderCredentialVersion>;
  listCredentialVersions(credentialId: string): Promise<ProviderCredentialVersion[]>;
  ...
  addPoolMember(member: ProviderCredentialPoolMember): Promise<ProviderCredentialPoolMember>;
  ...
  setAccountCapability(capability: ProviderAccountCapability): Promise<ProviderAccountCapability>;
  listAccountCapabilities(accountId: string): Promise<ProviderAccountCapability[]>;
