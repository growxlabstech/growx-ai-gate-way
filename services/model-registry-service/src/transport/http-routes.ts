  ...
  return async function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    try {
      ...
      if (pathname === "/health" || pathname === "/live" || pathname === "/ready") {
        return json(res, 200, { status: "ok", service: "model-registry-service" });
      }
      ...
      if (pathname === "/v1/openai/models" && method === "GET") {
        const allAdmin = await service.listAdminModels({ customerVisible: true, status: ["active", "deprecated"] });
        return json(res, 200, toOpenAIModelList(allAdmin.items));
      }

      const openAIModelMatch = pathname.match(/^\/v1\/openai\/models\/([^/]+)$/);
      ...
      if (pathname === "/internal/ops/models" && method === "GET") {
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.models.read", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }
      ...
      if (pathname === "/internal/ops/models" && method === "POST") {
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.models.write", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }
        ...
        if (!parsed.success) {
          return json(res, 400, { error: { code: "VALIDATION_ERROR", details: parsed.error.issues } });
        }

        const created = await service.createModel(parsed.data, auth.operatorId, requestId);
        return json(res, 201, created);
      }

      const opsModelDetailMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)$/);
      if (opsModelDetailMatch) {
        ...
        if (method === "PATCH") {
          ...
          const updated = await service.updateModel(id, parsed.data, auth.operatorId, requestId);
          return json(res, 200, updated);
        }
      }

      // Lifecycle actions: disable, deprecate, retire
      const opsDisableMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)\/disable$/);
      if (opsDisableMatch && method === "POST") {
        ...
        const updated = await service.disableModel(id, auth.operatorId, requestId);
        return json(res, 200, updated);
      }

      const opsDeprecateMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)\/deprecate$/);
      if (opsDeprecateMatch && method === "POST") {
        const id = decodeURIComponent(opsDeprecateMatch[1]!);
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.models.deprecate", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }
        ...
        const updated = await service.deprecateModel(id, parsed.data, auth.operatorId, requestId);
        return json(res, 200, updated);
      }

      const opsRetireMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)\/retire$/);
      if (opsRetireMatch && method === "POST") {
        const id = decodeURIComponent(opsRetireMatch[1]!);
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.models.retire", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }

        const updated = await service.retireModel(id, auth.operatorId, requestId);
        return json(res, 200, updated);
      }

      // 2. Provider Routes
      const opsCreateRouteMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)\/routes$/);
      if (opsCreateRouteMatch && method === "POST") {
        const modelId = decodeURIComponent(opsCreateRouteMatch[1]!);
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.routes.manage", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }

        const body = await readJson(req);
        const parsed = createProviderRouteRequestSchema.safeParse({ ...(body as any), modelId });
        if (!parsed.success) {
          return json(res, 400, { error: { code: "VALIDATION_ERROR", details: parsed.error.issues } });
        }

        const created = await service.addProviderRoute(parsed.data, auth.operatorId, requestId);
        return json(res, 201, created);
      }

      const opsUpdateRouteMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)\/routes\/([^/]+)$/);
      if (opsUpdateRouteMatch && method === "PATCH") {
        ...
        const updated = await service.updateProviderRoute(routeId, parsed.data, auth.operatorId, requestId);
        return json(res, 200, updated);
      }

      const opsDisableRouteMatch = pathname.match(/^\/internal\/ops\/models\/([^/]+)\/routes\/([^/]+)\/disable$/);
      if (opsDisableRouteMatch && method === "POST") {
        ...
        const updated = await service.disableProviderRoute(routeId, auth.operatorId, requestId);
        return json(res, 200, updated);
      }
      ...
      if (pathname === "/internal/ops/aliases" && method === "POST") {
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.aliases.manage", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }
        ...
        const created = await service.createAlias(parsed.data, auth.operatorId, requestId);
        return json(res, 201, created);
      }

      const opsUpdateAliasMatch = pathname.match(/^\/internal\/ops\/aliases\/([^/]+)$/);
      if (opsUpdateAliasMatch && method === "PATCH") {
        ...
        const updated = await service.updateAlias(aliasId, parsed.data, auth.operatorId, requestId);
        return json(res, 200, updated);
      }

      const opsRetireAliasMatch = pathname.match(/^\/internal\/ops\/aliases\/([^/]+)\/retire$/);
      if (opsRetireAliasMatch && method === "POST") {
        ...
        const updated = await service.retireAlias(aliasId, auth.operatorId, requestId);
        return json(res, 200, updated);
      }
      ...
      if (pathname === "/internal/ops/pricing" && method === "POST") {
        const auth = await privilegedAuth.authenticateAndAuthorize(req, "ops.pricing.manage", requestId);
        if (!auth.allowed) {
          return json(res, auth.status, { error: { code: auth.code, message: auth.message } });
        }
        ...
        const created = await service.addPricing(parsed.data, auth.operatorId, requestId);
        return json(res, 201, created);
      }

      // Route Not Found
      return json(res, 404, { error: { code: "NOT_FOUND", message: `Route '${method} ${pathname}' not found` } });
    } catch (err: any) {
