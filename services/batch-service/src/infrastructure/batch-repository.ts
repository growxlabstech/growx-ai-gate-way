...
export interface BatchRepository {
  createBatchJob(
    job: BatchJobRecord,
    reservation?: BatchExecutionReservationRecord,
  ...
  listBatchJobs(
    organizationId: string,
    query?: { status?: BatchJobStatus; limit?: number; cursor?: string },
  ): Promise<{
    data: BatchJobRecord[];
    hasMore: boolean;
  ...
  ): Promise<BatchIdempotencyRecord | null>;

  getReservation(
    batchId: string,
  ): Promise<BatchExecutionReservationRecord | null>;
  updateReservation(
    reservation: BatchExecutionReservationRecord,
  ): Promise<void>;
}
...
export class InMemoryBatchRepository implements BatchRepository {
  ...
  async getBatchItem(
    ...
    for (const item of this.items.values()) {
      if (
        ...
        (item.id === customIdOrId || item.customId === customIdOrId)
      ) {
        return { ...item };
      }
    }
      ...
      .filter(
        (i) =>
          ...
          (i.status === "pending" ||
            i.status === "queued" ||
            (i.status === "retry_wait" &&
              (!i.retryAfterAt || i.retryAfterAt <= now))),
      )
      .sort((a, b) => a.position - b.position);
  ...
  async claimRunnableBatchItemsFair(
    workerId: string,
    limit: number,
    leaseDurationMs: number,
    maxPerTenant: number,
