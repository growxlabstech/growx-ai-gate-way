...
test("returning user verifies OTP and returns to an authorized deep link", async ({ page }) => {
  await page.goto("/northstar/production/api-keys?tab=active");
  await expect(page).toHaveURL(/\/sign-in\?returnTo=/);
  await page.getByRole("textbox", { name: "EMAIL" }).fill("avery@northstar.example");
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  await expect(page.getByRole("heading", { name: "Verify email" })).toBeVisible();
  await page.getByRole("textbox", { name: "Six digit verification code" }).fill("111111");
  await page.waitForURL(/\/northstar\/production\/api-keys\?tab=active$/, { timeout: 30_000 });
  await expect(page.getByLabel("Current context")).toContainText("Production Gateway");
});

test("new user receives a real session and resumes at persisted onboarding state", async ({ page }) => {
  await page.goto("/sign-in");
  await page.getByRole("textbox", { name: "EMAIL" }).fill("new.user@example.com");
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  await page.getByRole("textbox", { name: "Six digit verification code" }).fill("222222");
  ...
  await expect(page.getByText("Nothing has been created or guessed.")).toBeVisible();
});

test("invalid and expired OTP states preserve recovery controls", async ({ page }) => {
  await page.goto("/sign-in");
  await page.getByRole("textbox", { name: "EMAIL" }).fill("avery@northstar.example");
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  const otp = page.getByRole("textbox", { name: "Six digit verification code" });
  await otp.fill("999999");
  await expect(page.locator(".auth-error")).toHaveText("Invalid code. Try again.");
...
test("rate limiting and change-email recovery are explicit", async ({ page }) => {
  await page.goto("/sign-in");
  const email = page.getByRole("textbox", { name: "EMAIL" });
  await email.fill("rate.limit@example.com");
  await page.getByRole("button", { name: "Continue", exact: true }).click();
  await expect(page.locator(".auth-error")).toHaveText("Too many attempts. Try again in 42s.");
...
test("legacy auth routes normalize and configured-only OAuth stays hidden", async ({ context, page }) => {
...
test("unsafe returnTo and expired sessions fail safely", async ({ context, page }) => {
