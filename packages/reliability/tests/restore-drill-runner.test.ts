...
describe('RestoreDrillRunner', () => {
  ...
  it('executes isolated restore drill and records verified RecoveryRun', async () => {
    const run = await runner.executeDrill({
      ...
      stateSnapshot: {
        walletBalances: [{ accountId: 'w_drill', balance: '10.00', ledgerSum: '10.00' }],
        apiKeys: [{ id: 'key_drill', secretHashPresent: true, orgId: 'org_drill' }],
        providerCredentials: [{ accountId: 'acc_drill', activeVersionCount: 1 }],
        batches: [],
        deletedResources: [],
      },
    });
