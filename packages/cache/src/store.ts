...
export class InMemoryExactCacheStore implements ExactCacheStore {
  private entries: Map<string, { entry: CacheEntry; expiresAtMs: number }> =
    new Map();
  ...
  public async invalidate(filter: InvalidationFilter): Promise<number> {
    ...
    for (const [key, record] of Array.from(this.entries.entries())) {
      ...
      if (
        filter.workspaceId &&
        record.entry.workspaceId !== filter.workspaceId
      ) {
        continue;
      }
      if (
        filter.canonicalModelId &&
        record.entry.canonicalModelId !== filter.canonicalModelId
      ) {
        continue;
      }
