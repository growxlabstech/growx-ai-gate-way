...
describe("Phase 25: Object Storage Provider Abstraction", () => {
  ...
  it("2. Supports range requests for streaming large audio/video payloads", async () => {
    ...
    await inMemoryProvider.putObject(key, fullData, { contentType: "audio/mpeg" });

    // Request bytes 4 to 9
    const rangeRes = await inMemoryProvider.getObject(key, { start: 4, end: 9 });
  ...
  it("3. Generates short-lived signed upload and download URLs", async () => {
    ...
    const uploadSigned = await inMemoryProvider.createSignedUploadUrl(key, { expiresInSeconds: 600 });
    ...
    const downloadSigned = await inMemoryProvider.createSignedDownloadUrl(key, { expiresInSeconds: 300 });
  ...
  it("4. Manages multipart upload lifecycle: create, upload parts, complete, abort", async () => {
    ...
    const mp = await inMemoryProvider.createMultipartUpload(key, { contentType: "application/pdf" });
    expect(mp.uploadId).toBeDefined();

    const part1 = await inMemoryProvider.uploadPart(key, mp.uploadId, 1, Buffer.from("Part 1 content. "));
    const part2 = await inMemoryProvider.uploadPart(key, mp.uploadId, 2, Buffer.from("Part 2 content."));

    expect(part1.partNumber).toBe(1);
    expect(part2.partNumber).toBe(2);

    const completed = await inMemoryProvider.completeMultipartUpload(key, mp.uploadId, [part1, part2]);
    expect(completed.contentLength).toBe(Buffer.from("Part 1 content. Part 2 content.").length);

    const fetched = await inMemoryProvider.getObject(key);
    expect(fetched.body.toString("utf8")).toBe("Part 1 content. Part 2 content.");
  });
  ...
  it("5. S3CompatibleObjectStorageProvider generates AWS SigV4 signed URLs", async () => {
    ...
    const signedUpload = await s3Provider.createSignedUploadUrl("org/org_1/files/f1/content", { expiresInSeconds: 900 });
    expect(signedUpload.uploadUrl).toContain("growx-customer-files.s3.us-east-1.amazonaws.com");
    expect(signedUpload.uploadUrl).toContain("X-Amz-Signature");

    const signedDownload = await s3Provider.createSignedDownloadUrl("org/org_1/files/f1/content", { expiresInSeconds: 900 });
    expect(signedDownload.downloadUrl).toContain("growx-customer-files.s3.us-east-1.amazonaws.com");
