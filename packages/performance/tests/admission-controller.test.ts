...
describe('AdmissionController', () => {
  ...
  it('throttles noisy tenant when tenant concurrency limit is exceeded', () => {
    ...
    const decision = controller.evaluateAdmission({ organizationId: 'org_noisy' });
    expect(decision.allowed).toBe(false);
    expect(decision.reason).toContain('Tenant concurrency limit');

    // Normal tenant can still be admitted
    const normalDecision = controller.evaluateAdmission({ organizationId: 'org_normal' });
  ...
  it('sheds low-priority BATCH requests when utilization crosses threshold', () => {
    ...
    const standardDec = controller.evaluateAdmission({ organizationId: 'org_std', priority: 'STANDARD' });
    expect(standardDec.allowed).toBe(true);

    // Batch request is shed
    const batchDec = controller.evaluateAdmission({ organizationId: 'org_batch', priority: 'BATCH' });
