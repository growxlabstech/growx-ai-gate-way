...
export interface IProviderOperationRepository {
  ...
  getByProviderOperationId(providerId: string, providerOperationId: string): Promise<ProviderOperation | null>;
  update(id: string, patch: Partial<ProviderOperation>): Promise<ProviderOperation>;
  ...
  releaseLease(id: string, leaseOwner: string, nextPollAt?: Date): Promise<void>;
...
export class InMemoryProviderOperationRepository implements IProviderOperationRepository {
  ...
  public async getByProviderOperationId(providerId: string, providerOperationId: string): Promise<ProviderOperation | null> {
    for (const op of this.operations.values()) {
      if (op.providerId === providerId && op.providerOperationId === providerOperationId) {
        return { ...op };
      }
    }
    return null;
  ...
  public async update(id: string, patch: Partial<ProviderOperation>): Promise<ProviderOperation> {
  ...
  public async claimDueForPolling(options: {
    ...
    for (const op of this.operations.values()) {
      ...
      const isLeaseFree = !op.leaseOwner || (op.leaseExpiresAt && op.leaseExpiresAt < now);
  ...
  public async releaseLease(id: string, leaseOwner: string, nextPollAt?: Date): Promise<void> {
    const op = this.operations.get(id);
  ...
  public async list(filter: ProviderOperationFilter): Promise<ProviderOperation[]> {
    const list = Array.from(this.operations.values());
    return list.filter((op) => {
      if (filter.organizationId && op.organizationId !== filter.organizationId) return false;
      if (filter.workspaceId && op.workspaceId !== filter.workspaceId) return false;
      if (filter.providerId && op.providerId !== filter.providerId) return false;
      if (filter.operationType && op.operationType !== filter.operationType) return false;
      if (filter.status) {
