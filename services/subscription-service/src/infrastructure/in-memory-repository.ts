...
export class InMemorySubscriptionRepository implements ISubscriptionRepository {
  ...
  async listPlans(filter?: {
    ...
    if (filter?.isPublic !== undefined)
      result = result.filter((p) => p.isPublic === filter.isPublic);
    return result
  ...
  async listSubscriptionsDueForRenewal(
    before: Date,
    limit: number,
  ...
  async updateSubscriptionPeriod(
    id: string,
    updates: Partial<SubscriptionPeriod>,
  ): Promise<void> {
  ...
  async saveEntitlementOverride(override: EntitlementOverride): Promise<void> {
    this.overrides.set(`${override.organizationId}:${override.key}`, {
      ...override,
    });
  }

  async getEntitlementOverrides(
    organizationId: string,
  ...
  async deleteEntitlementOverride(
    organizationId: string,
    key: string,
  ): Promise<void> {
