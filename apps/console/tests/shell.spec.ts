...
test("real context drives navigation, workspace switching, refresh, and sign out", async ({
  page,
}) => {
  ...
  await page
    ...
    .click();
  await expect(
    page.locator(".sidebar").getByRole("link", { name: "API keys" }),
  ).toHaveAttribute("aria-current", "page");
  await page.locator(".sidebar").getByRole("link", { name: "Usage" }).click();
  await expect(
    page.locator(".sidebar").getByRole("link", { name: "Usage" }),
  ).toHaveAttribute("aria-current", "page");

  await page
    .locator(".sidebar .switchers select")
    .nth(1)
  ...
  await expect(page.getByLabel("Current context")).not.toContainText(
    "Production Gateway",
  );
  await page.reload();
...
test("unauthorized tenant context fails without rendering the previous shell", async ({
  context,
  page,
}) => {
...
test("narrow navigation opens, closes with Escape, and returns focus", async ({
  page,
}) => {
  ...
  await trigger.click();
  await expect(page.getByLabel("Console navigation")).toHaveClass(
    /is-mobile-open/,
  );
  await page.keyboard.press("Escape");
  await expect(page.getByLabel("Console navigation")).not.toHaveClass(
    /is-mobile-open/,
  );
