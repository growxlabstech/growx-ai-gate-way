...
export function isValidJobTransition(from: BatchJobStatus, to: BatchJobStatus): boolean {
  if (from === to) return true;
  const allowed = VALID_JOB_TRANSITIONS[from];
  return allowed ? allowed.includes(to) : false;
}

export function assertValidJobTransition(from: BatchJobStatus, to: BatchJobStatus, batchId: string): void {
...
const VALID_ITEM_TRANSITIONS: Record<BatchItemStatus, readonly BatchItemStatus[]> = {
...
export function isValidItemTransition(from: BatchItemStatus, to: BatchItemStatus): boolean {
  if (from === to) return true;
  const allowed = VALID_ITEM_TRANSITIONS[from];
  return allowed ? allowed.includes(to) : false;
}

export function assertValidItemTransition(from: BatchItemStatus, to: BatchItemStatus, itemId: string): void {
