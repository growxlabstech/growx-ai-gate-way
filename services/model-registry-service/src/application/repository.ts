...
export interface IModelRegistryRepository {
  ...
  getModelByCanonicalId(canonicalId: string): Promise<CanonicalModelEntity | null>;
  updateModel(id: string, updates: Partial<CanonicalModelEntity>): Promise<CanonicalModelEntity>;
  listModels(filter?: ModelListFilter): Promise<{ items: CanonicalModelEntity[]; hasMore: boolean }>;
  ...
  getRouteByProviderModel(providerId: string, providerModelId: string, region: string): Promise<ProviderRouteEntity | null>;
  listRoutesByModelId(modelId: string): Promise<ProviderRouteEntity[]>;
  listAllRoutes(): Promise<ProviderRouteEntity[]>;
  updateRoute(id: string, updates: Partial<ProviderRouteEntity>): Promise<ProviderRouteEntity>;
  ...
  updateAlias(id: string, updates: Partial<ModelAliasEntity>): Promise<ModelAliasEntity>;

  // Model Pricing Versions
  createPricing(pricing: ModelPricingEntity): Promise<ModelPricingEntity>;
  getEffectivePricing(modelIdOrRouteId: string, timestamp?: Date): Promise<ModelPricingEntity | null>;
  listPricing(filter?: { modelId?: string | undefined; routeId?: string | undefined }): Promise<ModelPricingEntity[]>;
}
