...
function oauthError(value: string | string[] | undefined): string {
  ...
  if (code === "account_not_linked" || code === "account_conflict")
    return "This provider could not be linked to your existing account.";
...
export default async function Page({
  ...
  const returnToValue = Array.isArray(parameters.returnTo)
    ? parameters.returnTo[0]
    : parameters.returnTo;
  const returnTo = safeReturnTo(returnToValue);
  const context = await loadTenantContext();
  if (context.status === "ready" || context.status === "empty")
    redirect(resolveAccountDestination(context.context, returnTo));
  const enabledProviders = [
    process.env.AUTH_GOOGLE_ENABLED === "1" ? "google" : null,
    process.env.AUTH_GITHUB_ENABLED === "1" ? "github" : null,
  ...
  return (
    <AuthCard
      ...
      initialError={oauthError(parameters.error)}
    />
  );
}
