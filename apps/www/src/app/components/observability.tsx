...
export function Observability() {
  ...
  return (
    <section className="www-section">
      ...
      <p className="www-body">Every gateway request is instrumented with structured telemetry. Prometheus metrics, OpenTelemetry traces, and structured logging give you full visibility into model performance.</p>
      ...
      <p className="www-label" style={{ marginBottom: "12px" }}>Request inspection</p>
      <div className="www-inspection">
        {fields.map(f => (
          <div key={f.key} className="www-inspection-field">
            <div className="www-inspection-key">{f.key}</div>
            <div className={`www-inspection-val${f.accent ? " accent" : ""}`}>{f.val}</div>
          </div>
        ))}
      </div>
    </section>
