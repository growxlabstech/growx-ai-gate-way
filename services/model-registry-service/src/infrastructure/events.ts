...
export class DrizzleModelRegistryEvents implements IModelRegistryEvents {
  ...
  private async emitPrivilegedAudit(
    ...
    await (this.db as any)
      .insert((this.db as any).schema.privilegedAuditEvents)
      .values({
        id: auditId,
  ...
  async emitModelCreated(
    ...
    await this.emitOutbox(
      "model.created",
      { modelId: model.id, canonicalId: model.canonicalId },
      operatorId,
      requestId,
    );
    await this.emitPrivilegedAudit(
      "create",
      "canonical_model",
      model.id,
      operatorId,
  ...
  async emitRouteCreated(
    ...
    await this.emitOutbox(
      "model.route.created",
      { routeId: route.id, modelId: route.modelId },
      operatorId,
      requestId,
    );
    await this.emitPrivilegedAudit(
  ...
  async emitSecurityEvent(
    ...
    await (this.db as any)
      .insert((this.db as any).schema.securityEvents)
      .values({
        id: secId,
        type,
        severity,
...
export class InMemoryModelRegistryEvents implements IModelRegistryEvents {
  public readonly outbox: Array<{
    topic: string;
    payload: Record<string, unknown>;
    operatorId: string;
  }> = [];
  public readonly audit: Array<{
    action: string;
    resourceType: string;
    resourceId: string;
    operatorId: string;
  }> = [];
  public readonly security: Array<{
    type: string;
    severity: string;
    metadata: Record<string, unknown>;
  }> = [];
  ...
  async emitModelCreated(
    ...
    this.outbox.push({
      topic: "model.created",
      payload: { modelId: model.id, canonicalId: model.canonicalId },
      operatorId,
    });
    this.audit.push({
      action: "create",
      resourceType: "canonical_model",
      resourceId: model.id,
      operatorId,
    });
  }
  ...
  async emitModelDisabled(
    ...
    this.outbox.push({
      topic: "model.disabled",
      payload: { modelId, canonicalId },
      operatorId,
    });
    this.audit.push({
      action: "disable",
  ...
  async emitAliasCreated(
    ...
    this.outbox.push({
      topic: "model.alias.created",
      payload: { aliasId: alias.id, alias: alias.alias },
      operatorId,
    });
    this.audit.push({
      action: "create",
  ...
  async emitAliasRetired(
    ...
    this.outbox.push({
      topic: "model.alias.retired",
      payload: { aliasId, alias: aliasName },
      operatorId,
    });
    this.audit.push({
      action: "retire",
  ...
  async emitPricingCreated(
    ...
    this.outbox.push({
      topic: "model.pricing.created",
      payload: { pricingId: pricing.id },
      operatorId,
    });
    this.audit.push({
      action: "create",
