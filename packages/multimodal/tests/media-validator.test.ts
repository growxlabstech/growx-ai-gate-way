...
describe("MediaValidator", () => {
  ...
  it("rejects unsupported image MIME types", () => {
    expect(() => MediaValidator.validateImageMime("image/svg+xml")).toThrow(
      MediaValidationError,
    );
    expect(() => MediaValidator.validateImageMime("application/pdf")).toThrow(
      MediaValidationError,
    );
  });
  ...
  it("validates safe image dimensions", () => {
    expect(() =>
      MediaValidator.validateImageDimensions(1024, 1024),
    ).not.toThrow();
    expect(() =>
      MediaValidator.validateImageDimensions(4096, 4096),
    ).not.toThrow();
  });

  it("rejects zero or negative image dimensions", () => {
    expect(() => MediaValidator.validateImageDimensions(0, 100)).toThrow(
      MediaValidationError,
    );
    expect(() => MediaValidator.validateImageDimensions(100, -5)).toThrow(
      MediaValidationError,
    );
  });
  ...
  it("detects and blocks pixel bombs (extremely large pixel count)", () => {
    ...
    expect(() =>
      MediaValidator.validateImageDimensions(10000, 10000, 64_000_000),
    ).toThrow(PixelBombError);
  });
  ...
  it("rejects unsupported audio MIME types", () => {
    expect(() => MediaValidator.validateAudioMime("video/mp4")).toThrow(
      MediaValidationError,
    );
    expect(() => MediaValidator.validateAudioMime("text/plain")).toThrow(
      MediaValidationError,
    );
  });
  ...
  it("rejects negative or excessive audio duration", () => {
    expect(() => MediaValidator.validateAudioDuration(-10)).toThrow(
      MediaValidationError,
    );
    expect(() => MediaValidator.validateAudioDuration(5000, 3600)).toThrow(
      MediaValidationError,
    );
  });
  ...
  it("rejects oversized data URIs", () => {
    ...
    expect(() => MediaValidator.validateBase64DataUri(dataUri, 500)).toThrow(
      MediaValidationError,
    );
  });
});
