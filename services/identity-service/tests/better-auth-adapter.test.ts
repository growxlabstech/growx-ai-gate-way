...
describe("GrowX Better Auth compatibility adapter", () => {
  ...
  it("protects session tokens on create and lookup while returning the logical token", async () => {
    ...
    const baseFactory = (() => ({
      create: async ({ data }: { data: Record<string, unknown> }) => {
        writes.push(data);
        return { ...data, expiresAt: new Date(Date.now() + 60_000) };
      },
      findOne: async ({ where }: { where: unknown[] }) => {
        lookups.push(where);
        return writes[0] ?? null;
      },
    })) as unknown as Factory;
    const adapter = growxBetterAuthAdapter(baseFactory, {
      sessionPepper: "s".repeat(32),
      providerEncryptionSecret: "e".repeat(32),
    })({});
    const raw = "browser-only-session-token";
    const created = await adapter.create({
      model: "sessions",
      data: { id: "ses_1", token: raw },
    });
    ...
    expect(lookups).toEqual([
      [{ field: "token", value: protectSessionToken(raw, "s".repeat(32)) }],
    ]);
  ...
  it.each([
    ...
    const baseFactory = (() => ({
      findOne: async () => ({ id: "ses_1", token: "protected", ...stored }),
    })) as unknown as Factory;
    const adapter = growxBetterAuthAdapter(baseFactory, {
      sessionPepper: "s".repeat(32),
      providerEncryptionSecret: "e".repeat(32),
  ...
  it("encrypts OAuth credentials at rest and reveals them only at the adapter boundary", async () => {
    ...
    ).resolves.toMatchObject(logical);
  });
});
