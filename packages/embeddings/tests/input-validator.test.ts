...
describe("Embedding Input Validator", () => {
  ...
  it("rejects empty string input", () => {
    ...
    expect(() => normalizeEmbeddingInput("   ")).toThrow(
  ...
  it("rejects array containing empty string items", () => {
    expect(() => normalizeEmbeddingInput(["valid", ""])).toThrow(
      EmbeddingValidationError,
    );
  });
  ...
  it("estimates input tokens reasonably", () => {
    ...
    expect(
      estimateInputTokens("this is a longer sentence with several words"),
    ).toBeGreaterThanOrEqual(7);
  });

  it("enforces maximum batch item limits", () => {
    const items = Array.from({ length: 15 }, (_, i) => "item " + i);
    expect(() => validateEmbeddingInput(items, defaultLimits)).toThrow(
      EmbeddingValidationError,
    );
  });
  ...
  it("enforces per-item token limits", () => {
    ...
    expect(() => validateEmbeddingInput([longItem], defaultLimits)).toThrow(
      EmbeddingValidationError,
    );
  });
  ...
  it("calculates total estimated tokens and bytes correctly", () => {
    ...
    const { totalEstimatedTokens, totalBytes } = validateEmbeddingInput(
      inputs,
      defaultLimits,
    );
