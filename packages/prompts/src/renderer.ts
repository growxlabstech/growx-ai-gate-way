...
export class PromptTemplateRenderer {
  public static calculateContentHash(
    ...
    const canonicalPayload = {
      messages: messages ?? [],
      template: template ?? "",
      variableSchema: [...variableSchema].sort((a, b) => a.name.localeCompare(b.name)),
  ...
  public static calculateRenderedHash(renderedMessages: Array<{ role: string; content: string }>): string {
  ...
  public static validateAndCoerceVariables(
    ...
    if (!options.allowExtraVariables) {
      for (const key of Object.keys(providedVariables)) {
        if (!schemaMap.has(key)) {
          throw new PromptValidationError(`Unknown prompt variable '${key}'. Not declared in schema`, {
            variable: key,
          });
        }
      }
    ...
    for (const [name, def] of schemaMap.entries()) {
      const rawVal = providedVariables[name] !== undefined ? providedVariables[name] : def.defaultValue;

      if (rawVal === undefined || rawVal === null) {
        if (def.required) {
          throw new PromptValidationError(`Missing required prompt variable '${name}'`, {
            variable: name,
          });
        }
        continue;
      ...
      switch (def.type) {
        case "string": {
          if (typeof rawVal !== "string") {
            throw new PromptValidationError(`Variable '${name}' must be a string, received ${typeof rawVal}`, {
              variable: name,
            });
          }
          if (def.maxLength && rawVal.length > def.maxLength) {
            throw new PromptValidationError(`Variable '${name}' exceeds maxLength of ${def.maxLength}`, {
              variable: name,
            });
          }
          if (def.enum && !def.enum.includes(rawVal)) {
            throw new PromptValidationError(`Variable '${name}' value '${rawVal}' is not in allowed enum: ${def.enum.join(", ")}`, {
              variable: name,
            });
          }
          if (Buffer.byteLength(rawVal, "utf8") > maxVarBytes) {
            throw new PromptValidationError(`Variable '${name}' exceeds maximum allowed size of ${maxVarBytes} bytes`, {
              variable: name,
            });
          }
        ...
        case "number": {
          if (typeof rawVal !== "number" || !Number.isFinite(rawVal)) {
            throw new PromptValidationError(`Variable '${name}' must be a finite number`, {
              variable: name,
            });
          }
          validated[name] = rawVal;
        ...
        case "boolean": {
          if (typeof rawVal !== "boolean") {
            throw new PromptValidationError(`Variable '${name}' must be a boolean`, {
              variable: name,
            });
          }
          validated[name] = rawVal;
        ...
        case "array": {
          if (!Array.isArray(rawVal)) {
            throw new PromptValidationError(`Variable '${name}' must be an array`, {
              variable: name,
            });
          }
        ...
        case "object": {
          if (typeof rawVal !== "object" || rawVal === null || Array.isArray(rawVal)) {
            throw new PromptValidationError(`Variable '${name}' must be an object`, {
              variable: name,
            });
          }
          const serialized = JSON.stringify(rawVal);
  ...
  public static interpolate(template: string, variables: Record<string, unknown>): string {
  ...
  public static render(
    ...
    if (version.messages && version.messages.length > 0) {
      for (const msg of version.messages) {
        const renderedContent = this.interpolate(msg.contentTemplate, validated);
      ...
      throw new PromptRenderError("Prompt version has neither messages nor template content");
    }

    // Check total render size
    const totalSize = renderedMessages.reduce((sum, m) => sum + Buffer.byteLength(m.content, "utf8"), 0);
    if (totalSize > maxTotalBytes) {
      throw new PromptRenderError(`Total rendered prompt size (${totalSize} bytes) exceeds limit of ${maxTotalBytes} bytes`, {
        totalSize,
        maxTotalBytes,
      });
    }

    const renderedText = renderedMessages.map(m => `[${m.role}] ${m.content}`).join("\n\n");
    const contentHash = version.contentHash || this.calculateContentHash(version.messages, version.template, version.variableSchema, version.outputSchema);
