...
export class DataResidencyViolationError extends GovernancePolicyError {
  constructor(public requiredRegion: string, public routeRegion: string) {
    super(
...
export class RetentionHoldActiveError extends GovernancePolicyError {
  constructor(public holdId: string, public reason: string) {
    super(
...
export class DeletionProcessorError extends GovernancePolicyError {
  constructor(public processor: string, message: string) {
    super("DELETION_PROCESSOR_FAILED", `Deletion processor '${processor}' failed: ${message}`);
