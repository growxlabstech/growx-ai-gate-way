...
export const requestStatus = pgEnum("gateway_request_status", [
  "accepted",
  "routing",
  "executing",
  "streaming",
...
export const attemptStatus = pgEnum("provider_attempt_status", [
  "started",
  "streaming",
  "completed",
  "failed",
...
export const subscriptionFundingMode = pgEnum("subscription_funding_mode", [
  "manual",
  "free",
  "external_payment_future",
  "enterprise_contract",
]);
export const subscriptionPeriodStatus = pgEnum("subscription_period_status", [
  "pending",
  "active",
  "renewed",
  "expired",
...
export const fileSafetyState = pgEnum("file_safety_state", [
  "not_scanned",
  "pending",
  "clean",
  "rejected",
...
export const fileUploadSessionStatus = pgEnum("file_upload_session_status", [
  "pending",
  "active",
  "completed",
  "aborted",
...
export const fileUploadType = pgEnum("file_upload_type", [
  "single",
  "multipart",
]);
export const fileProviderReferenceStatus = pgEnum(
  "file_provider_reference_status",
...
export const batchItemStatus = pgEnum("batch_item_status", [
  "pending",
  "queued",
  "running",
  "succeeded",
...
export const batchChunkStatus = pgEnum("batch_chunk_status", [
  "pending",
  "in_progress",
  "completed",
  "failed",
]);
export const batchReservationStatus = pgEnum("batch_reservation_status", [
  "reserved",
  "partially_settled",
  "settled",
  "released",
]);
export const routingWorkloadType = pgEnum("routing_workload_type", [
...
export const routingLatencyClass = pgEnum("routing_latency_class", [
  "ultra_low",
  "interactive",
  "standard",
  "throughput",
]);
export const routingObjective = pgEnum("routing_objective", [
  "balanced",
  "lowest_latency",
  "lowest_cost",
  "highest_reliability",
...
export const trafficControlMode = pgEnum("traffic_control_mode", [
  "active",
  "draining",
  "disabled",
  "canary",
...
export const verificationTokens = pgTable(
  ...
  (table) => [
    uniqueIndex("verification_token_hash_unique").on(table.tokenHash),
...
export const verifications = pgTable(
  ...
  (table) => [
    ...
    index("verifications_expiry_idx").on(table.expiresAt),
  ],
);
...
export const organizations = pgTable(
  "organizations",
  {
    ...
    status: organizationStatus().notNull().default("trial"),
...
export const organizationMembers = pgTable(
  ...
  (table) => [
    uniqueIndex("organization_members_tenant_user_unique").on(
...
export const teamMembers = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.teamId, table.organizationMemberId] }),
  ],
);
...
export const workspaces = pgTable(
  "workspaces",
  {
    ...
    region: text().notNull().default("auto"),
  ...
  (table) => [
    uniqueIndex("workspaces_org_slug_unique").on(
      table.organizationId,
      table.slug,
    ),
...
export const workspaceMembers = pgTable(
  ...
  (table) => [
    uniqueIndex("workspace_members_tenant_user_unique").on(
      table.organizationId,
      table.workspaceId,
      table.userId,
    ),
...
export const workspaceTeams = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.workspaceId, table.teamId] })],
);
export const environments = pgTable(
  "environments",
  {
    ...
    name: text().notNull(),
    slug: text().notNull(),
    type: environmentType().notNull(),
    status: workspaceStatus().notNull().default("active"),
    ...timestamps,
  },
  (table) => [
    uniqueIndex("environments_workspace_slug_unique").on(
      table.organizationId,
      table.workspaceId,
      table.slug,
    ),
...
export const apiKeys = pgTable(
  "api_keys",
  {
    ...
    status: apiKeyStatus().notNull().default("active"),
    createdBy: text("created_by")
      .notNull()
      .references(() => users.id, { onDelete: "restrict" }),
  ...
  (table) => [
    ...
    index("api_keys_environment_idx").on(table.environmentId),
    index("api_keys_status_idx").on(table.status),
    index("api_keys_expiry_idx").on(table.status, table.expiresAt),
    index("api_keys_last_used_idx").on(table.lastUsedAt),
    index("api_keys_org_workspace_idx").on(
      table.organizationId,
      table.workspaceId,
    ),
    index("api_keys_workspace_environment_idx").on(
      table.workspaceId,
      table.environmentId,
    ),
  ],
...
export const apiKeyPermissions = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.apiKeyId, table.permission] })],
);
export const apiKeyModelRules = pgTable(
  "api_key_model_rules",
  {
    id: text().primaryKey(),
    apiKeyId: text("api_key_id")
    ...
    category: text(),
    maximumCostMinor: integer("maximum_cost_minor"),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
      .defaultNow(),
  },
  (table) => [index("api_key_model_rules_key_idx").on(table.apiKeyId)],
);
export const apiKeyRateLimits = pgTable(
  ...
  (table) => [
    uniqueIndex("api_key_rate_limits_key_window_unique").on(
      table.apiKeyId,
      table.window,
    ),
...
export const apiKeySpendingLimits = pgTable(
  "api_key_spending_limits",
  {
    apiKeyId: text("api_key_id")
      .primaryKey()
      .references(() => apiKeys.id, { onDelete: "cascade" }),
    mode: budgetMode().notNull(),
    perRequestMinor: integer("per_request_minor"),
    dailyMinor: integer("daily_minor"),
  ...
  (table) => [
    check(
      "api_key_spending_currency_length",
      sql`length(${table.currency}) = 3`,
    ),
  ],
);
export const apiKeyIpAllowlists = pgTable(
  ...
  (table) => [
    uniqueIndex("api_key_ip_allowlists_key_cidr_unique").on(
      table.apiKeyId,
      table.cidr,
    ),
  ],
);
export const apiKeyUsageSnapshots = pgTable(
  ...
  (table) => [
    uniqueIndex("api_key_usage_period_unique").on(
      table.apiKeyId,
      table.periodStart,
      table.periodEnd,
    ),
...
export const providerConnections = pgTable(
  ...
  (table) => [
    index("provider_connections_scope_idx").on(
      table.organizationId,
      table.workspaceId,
      table.environmentId,
    ),
...
export const providerCredentials = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("provider_credentials_provider_name_env_unique").on(
      table.providerId,
      table.name,
      table.environment,
    ),
...
export const canonicalModels = pgTable(
  ...
  (table) => [
    ...
    index("canonical_models_visibility_idx").on(
      table.customerVisible,
      table.routingEligible,
    ),
  ],
);
...
export const modelProviderRoutes = pgTable(
  ...
  (table) => [
    uniqueIndex("model_provider_routes_provider_model_region_unique").on(
      table.providerId,
      table.providerModelId,
      table.region,
    ),
    index("model_provider_routes_model_idx").on(table.modelId),
    index("model_provider_routes_status_idx").on(
      table.status,
      table.routingEligible,
    ),
  ],
...
export const canonicalModelPricing = pgTable(
  ...
  {
    id: text().primaryKey(),
    modelId: text("model_id").references(() => canonicalModels.id, {
      onDelete: "cascade",
    }),
    routeId: text("route_id").references(() => modelProviderRoutes.id, {
      onDelete: "cascade",
    }),
    pricingType: text("pricing_type").notNull().default("standard"),
    inputPricePerMillionMinor: bigint("input_price_per_million_minor", {
      mode: "bigint",
    }).notNull(),
    outputPricePerMillionMinor: bigint("output_price_per_million_minor", {
      mode: "bigint",
    }).notNull(),
    cachedInputPricePerMillionMinor: bigint(
      "cached_input_price_per_million_minor",
      { mode: "bigint" },
    ),
    reasoningPricePerMillionMinor: bigint("reasoning_price_per_million_minor", {
      mode: "bigint",
    }),
    currency: text().notNull().default("USD"),
  ...
  (table) => [
    index("canonical_model_pricing_model_idx").on(
      table.modelId,
      table.effectiveFrom,
    ),
    index("canonical_model_pricing_route_idx").on(
      table.routeId,
      table.effectiveFrom,
    ),
  ],
...
export const providerModels = pgTable(
  "provider_models",
  {
    ...
    displayName: text("display_name").notNull(),
    description: text().notNull().default(""),
    status: modelStatus().notNull().default("active"),
    contextWindow: integer("context_window").notNull(),
    maxOutputTokens: integer("max_output_tokens").notNull(),
    ...timestamps,
  },
  (table) => [
    uniqueIndex("provider_models_public_unique").on(table.publicModelId),
    uniqueIndex("provider_models_native_unique").on(
      table.providerId,
      table.providerModelId,
    ),
    index("provider_models_status_idx").on(table.status),
  ],
);
export const providerModelCapabilities = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.providerModelId, table.capability] }),
  ],
);
export const providerHealthSnapshots = pgTable(
  "provider_health_snapshots",
  {
    ...
    successRateBasisPoints: integer("success_rate_basis_points"),
    sampledAt: timestamp("sampled_at", { withTimezone: true }).notNull(),
  },
  (table) => [
    index("provider_health_provider_sample_idx").on(
      table.providerId,
      table.sampledAt,
    ),
  ],
);
export const providerPricingVersions = pgTable("provider_pricing_versions", {
  id: text().primaryKey(),
  providerModelId: text("provider_model_id")
    .notNull()
    .references(() => providerModels.id, { onDelete: "restrict" }),
  currency: text().notNull(),
  inputPerMillionMinor: integer("input_per_million_minor").notNull(),
  outputPerMillionMinor: integer("output_per_million_minor").notNull(),
  cachedPerMillionMinor: integer("cached_per_million_minor"),
...
export const modelAliasVersions = pgTable(
  "model_alias_versions",
  {
    id: text().primaryKey(),
    aliasId: text("alias_id")
      .notNull()
      .references(() => modelAliases.id, { onDelete: "restrict" }),
  ...
  (table) => [
    uniqueIndex("model_alias_versions_unique").on(table.aliasId, table.version),
  ],
);
export const modelAvailability = pgTable(
  "model_availability",
  {
    ...
    providerModelId: text("provider_model_id")
      ...
      .references(() => providerModels.id, { onDelete: "cascade" }),
    region: text().notNull(),
    environmentType: environmentType("environment_type").notNull(),
    available: boolean().notNull().default(true),
    ...timestamps,
  },
  (table) => [
    uniqueIndex("model_availability_scope_unique").on(
      table.providerModelId,
      table.region,
...
export const modelDeprecations = pgTable("model_deprecations", {
  providerModelId: text("provider_model_id")
    .primaryKey()
    .references(() => providerModels.id, { onDelete: "cascade" }),
...
export const routingPolicyVersions = pgTable(
  "routing_policy_versions",
  {
    id: text().primaryKey(),
    routingPolicyId: text("routing_policy_id")
      .notNull()
      .references(() => routingPolicies.id, { onDelete: "restrict" }),
    version: text().notNull(),
  ...
  (table) => [
    uniqueIndex("routing_policy_version_unique").on(
      table.routingPolicyId,
      table.version,
    ),
  ],
);
export const routingRules = pgTable("routing_rules", {
  id: text().primaryKey(),
  routingPolicyVersionId: text("routing_policy_version_id")
    .notNull()
    .references(() => routingPolicyVersions.id, { onDelete: "cascade" }),
  priority: integer().notNull(),
  condition: jsonb().notNull(),
  target: jsonb().notNull(),
});
export const fallbackChains = pgTable("fallback_chains", {
  id: text().primaryKey(),
  routingPolicyVersionId: text("routing_policy_version_id")
    .notNull()
    .references(() => routingPolicyVersions.id, { onDelete: "cascade" }),
...
export const gatewayRequests = pgTable(
  "gateway_requests",
  {
    ...
    stream: boolean().notNull().default(false),
  ...
  (table) => [
    index("gateway_requests_tenant_created_idx").on(
      table.organizationId,
      table.workspaceId,
      table.createdAt,
    ),
    index("gateway_requests_key_idx").on(table.apiKeyId),
  ],
);
export const routingDecisions = pgTable(
  "routing_decisions",
  {
    ...
    providerModelId: text("provider_model_id")
      .notNull()
      .references(() => providerModels.id, { onDelete: "restrict" }),
    routingPolicyVersionId: text("routing_policy_version_id").references(
      () => routingPolicyVersions.id,
      { onDelete: "restrict" },
    ),
  ...
  (table) => [
    uniqueIndex("routing_decisions_request_unique").on(table.requestId),
  ],
);
export const providerAttempts = pgTable(
  ...
  (table) => [
    uniqueIndex("provider_attempts_request_number_unique").on(
      table.requestId,
      table.attemptNumber,
    ),
  ],
...
export const usageRecords = pgTable("usage_records", {
  ...
  providerPricingVersionId: text("provider_pricing_version_id").references(
    () => providerPricingVersions.id,
    { onDelete: "restrict" },
  ),
  providerCurrency: text("provider_currency"),
  estimatedProviderCostMinor: integer("estimated_provider_cost_minor"),
  createdAt: timestamp("created_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
});
export const tokenUsageRecords = pgTable("token_usage_records", {
...
export const latencyRecords = pgTable("latency_records", {
  requestId: text("request_id")
    .primaryKey()
...
export const errorRecords = pgTable(
  "error_records",
  {
    ...
    requestId: text("request_id")
      .notNull()
      .references(() => gatewayRequests.id, { onDelete: "restrict" }),
    providerAttemptId: text("provider_attempt_id").references(
      () => providerAttempts.id,
      { onDelete: "restrict" },
    ),
    code: text().notNull(),
  ...
  (table) => [index("error_records_request_idx").on(table.requestId)],
);
...
export const usageEvents = pgTable(
  "usage_events",
  {
    ...
    requestId: text("request_id")
      .notNull()
      .references(() => gatewayRequests.id, { onDelete: "restrict" }),
    attemptId: text("attempt_id").references(() => providerAttempts.id, {
      onDelete: "restrict",
    }),
    organizationId: text("organization_id")
      .notNull()
  ...
  (table) => [
    ...
    index("usage_events_tenant_occurred_idx").on(
      table.organizationId,
      table.workspaceId,
      table.occurredAt,
    ),
...
export const usageReconciliations = pgTable(
  "usage_reconciliations",
  {
    ...
    differenceQuantity: bigint("difference_quantity", {
      mode: "bigint",
    }).notNull(),
    usageType: text("usage_type").notNull(),
    reason: text().notNull(),
...
export const usageHourlyAggregates = pgTable(
  ...
  (table) => [
    index("usage_hourly_agg_tenant_bucket_idx").on(
      table.organizationId,
      table.workspaceId,
      table.bucketStart,
    ),
...
export const usageDailyAggregates = pgTable(
  ...
  (table) => [
    index("usage_daily_agg_tenant_bucket_idx").on(
      table.organizationId,
      table.workspaceId,
      table.bucketStart,
    ),
...
export const analyticsHourlyRollups = pgTable(
  "analytics_hourly_rollups",
  {
    ...
    reasoningTokens: bigint("reasoning_tokens", { mode: "bigint" })
      .notNull()
      .default(sql`0`),
    providerInputTokens: bigint("provider_input_tokens", { mode: "bigint" })
      .notNull()
      .default(sql`0`),
    providerOutputTokens: bigint("provider_output_tokens", { mode: "bigint" })
      .notNull()
      .default(sql`0`),
    providerTotalTokens: bigint("provider_total_tokens", { mode: "bigint" })
      .notNull()
      .default(sql`0`),
    latencySketch: jsonb("latency_sketch").notNull(),
    ttftSketch: jsonb("ttft_sketch").notNull(),
    errorCounts: jsonb("error_counts")
      .notNull()
      .default(sql`'{}'::jsonb`),
    policyDenialCounts: jsonb("policy_denial_counts")
      .notNull()
      .default(sql`'{}'::jsonb`),
    quotaDenialCounts: jsonb("quota_denial_counts")
      .notNull()
      .default(sql`'{}'::jsonb`),
    updatedAt: timestamp("updated_at", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    index("analytics_hourly_tenant_bucket_idx").on(
      table.organizationId,
      table.workspaceId,
      table.bucketStart,
    ),
    index("analytics_hourly_model_idx").on(
      table.canonicalModelId,
      table.bucketStart,
    ),
    index("analytics_hourly_provider_idx").on(
      table.providerId,
      table.bucketStart,
    ),
  ],
...
export const analyticsDailyRollups = pgTable(
  ...
  (table) => [
    index("analytics_daily_tenant_bucket_idx").on(
      table.organizationId,
      table.workspaceId,
      table.bucketStart,
    ),
    index("analytics_daily_model_idx").on(
      table.canonicalModelId,
      table.bucketStart,
    ),
    index("analytics_daily_provider_idx").on(
      table.providerId,
      table.bucketStart,
    ),
  ],
...
export const analyticsCheckpoints = pgTable("analytics_checkpoints", {
  id: text().primaryKey(),
  ...
  processedEventsCount: bigint("processed_events_count", { mode: "bigint" })
    .notNull()
    .default(sql`0`),
  updatedAt: timestamp("updated_at", { withTimezone: true })
    .notNull()
...
export const analyticsAnomalies = pgTable(
  ...
  (table) => [
    index("analytics_anomalies_tenant_detected_idx").on(
      table.organizationId,
      table.workspaceId,
      table.detectedAt,
    ),
    index("analytics_anomalies_type_idx").on(
      table.anomalyType,
      table.detectedAt,
    ),
  ],
...
export const routingConditions = pgTable("routing_conditions", {
  id: text().primaryKey(),
...
export const routingActions = pgTable("routing_actions", {
  id: text().primaryKey(),
  routingRuleId: text("routing_rule_id")
    .notNull()
    .references(() => routingRules.id, { onDelete: "cascade" }),
...
export const trafficAllocations = pgTable("traffic_allocations", {
  ...
  allocation: jsonb().notNull(),
  createdAt: timestamp("created_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
});
export const providerWeights = pgTable(
  "provider_weights",
  {
    routingPolicyVersionId: text("routing_policy_version_id")
      .notNull()
  ...
  (table) => [
    primaryKey({ columns: [table.routingPolicyVersionId, table.providerId] }),
    check(
...
export const routingCandidateScores = pgTable(
  "routing_candidate_scores",
  {
    ...
    providerModelId: text("provider_model_id")
      .notNull()
      .references(() => providerModels.id, { onDelete: "restrict" }),
    costScoreBasisPoints: integer("cost_score_basis_points").notNull(),
    latencyScoreBasisPoints: integer("latency_score_basis_points").notNull(),
    reliabilityScoreBasisPoints: integer(
      "reliability_score_basis_points",
    ).notNull(),
    capacityScoreBasisPoints: integer("capacity_score_basis_points").notNull(),
    preferenceScoreBasisPoints: integer(
      "preference_score_basis_points",
    ).notNull(),
    finalScoreBasisPoints: integer("final_score_basis_points").notNull(),
    eligible: boolean().notNull(),
    rejectionReason: text("rejection_reason"),
  },
  (table) => [
    index("routing_candidate_scores_decision_idx").on(table.routingDecisionId),
  ],
);
export const providerCapacityLimits = pgTable("provider_capacity_limits", {
  id: text().primaryKey(),
  providerId: text("provider_id")
    .notNull()
    .references(() => providers.id, { onDelete: "cascade" }),
  ...
  safetyMarginBasisPoints: integer("safety_margin_basis_points")
    .notNull()
    .default(8000),
  ...timestamps,
});
export const providerCapacitySnapshots = pgTable(
  ...
  {
    id: text().primaryKey(),
    capacityLimitId: text("capacity_limit_id")
      .notNull()
      .references(() => providerCapacityLimits.id, { onDelete: "cascade" }),
    currentConcurrency: integer("current_concurrency").notNull(),
    requestsPerMinute: integer("requests_per_minute").notNull(),
    tokensPerMinute: integer("tokens_per_minute").notNull(),
    utilizationBasisPoints: integer("utilization_basis_points").notNull(),
    state: text().notNull(),
    recordedAt: timestamp("recorded_at", { withTimezone: true }).notNull(),
  },
  (table) => [
    index("provider_capacity_snapshot_limit_time_idx").on(
      table.capacityLimitId,
      table.recordedAt,
    ),
  ],
);
export const providerCircuitStates = pgTable(
  ...
  (table) => [
    uniqueIndex("provider_circuit_scope_unique").on(
      table.providerId,
      table.providerModelId,
      table.connectionId,
    ),
...
export const providerMaintenanceWindows = pgTable(
  ...
  (table) => [
    index("provider_maintenance_provider_time_idx").on(
      table.providerId,
      table.startsAt,
      table.endsAt,
    ),
...
export const cacheRecords = pgTable(
  ...
  (table) => [
    uniqueIndex("cache_records_tenant_fingerprint_unique").on(
      table.organizationId,
      table.workspaceId,
      table.fingerprint,
    ),
    index("cache_records_expiry_idx").on(table.expiresAt),
  ],
);
...
export const billingAccounts = pgTable(
  "billing_accounts",
  {
    ...
    currency: text().notNull(),
    status: text().notNull().default("active"),
    overagePolicy: text("overage_policy").notNull().default("none"),
    graceCredits: bigint("grace_credits", { mode: "bigint" })
      .notNull()
      .default(sql`0`),
    ...timestamps,
  },
  (table) => [
    check("billing_accounts_currency", sql`length(${table.currency}) = 3`),
  ],
);
...
export const pricingRules = pgTable(
  ...
  (table) => [
    index("pricing_rules_version_level_idx").on(
...
export const modelPrices = pgTable(
  "model_prices",
  {
    ...
    pricingVersionId: text("pricing_version_id")
      ...
      .references(() => pricingVersions.id, { onDelete: "restrict" }),
    publicModelId: text("public_model_id").notNull(),
    inputPerMillionMinor: bigint("input_per_million_minor", {
      mode: "bigint",
    }).notNull(),
    outputPerMillionMinor: bigint("output_per_million_minor", {
      mode: "bigint",
    }).notNull(),
    cachedPerMillionMinor: bigint("cached_per_million_minor", {
      mode: "bigint",
    })
      .notNull()
      .default(sql`0`),
    reasoningPerMillionMinor: bigint("reasoning_per_million_minor", {
      mode: "bigint",
    })
      .notNull()
      .default(sql`0`),
  ...
  (table) => [
    uniqueIndex("model_prices_version_model_unique").on(
      table.pricingVersionId,
      table.publicModelId,
    ),
  ],
);
export const creditConversionVersions = pgTable("credit_conversion_versions", {
  id: text().primaryKey(),
  version: integer().notNull().unique(),
  currency: text().notNull(),
  creditsNumerator: bigint("credits_numerator", { mode: "bigint" }).notNull(),
  moneyMinorDenominator: bigint("money_minor_denominator", {
    mode: "bigint",
  }).notNull(),
  effectiveFrom: timestamp("effective_from", { withTimezone: true }).notNull(),
  effectiveUntil: timestamp("effective_until", { withTimezone: true }),
...
export const creditWallets = pgTable("credit_wallets", {
  ...
  organizationId: text("organization_id")
    ...
    .unique(),
  currency: text().notNull(),
  status: walletStatus().notNull().default("active"),
  cachedAvailableCredits: bigint("cached_available_credits", { mode: "bigint" })
    .notNull()
    .default(sql`0`),
  cachedReservedCredits: bigint("cached_reserved_credits", { mode: "bigint" })
    .notNull()
    .default(sql`0`),
  version: integer().notNull().default(1),
  ...timestamps,
...
export const creditGrants = pgTable(
  "credit_grants",
  {
    ...
    remainingCredits: bigint("remaining_credits", { mode: "bigint" }).notNull(),
  ...
  (table) => [
    index("credit_grants_wallet_expiry_idx").on(
      table.walletId,
      table.expiresAt,
    ),
    ...
    check(
      "credit_grants_nonnegative",
      sql`${table.credits} >= 0 AND ${table.remainingCredits} >= 0 AND ${table.remainingCredits} <= ${table.credits}`,
    ),
  ],
);
export const creditReservations = pgTable(
  ...
  (table) => [
    ...
    index("credit_reservations_wallet_status_idx").on(
      table.walletId,
      table.status,
    ),
  ],
...
export const paymentCustomers = pgTable(
  ...
  (table) => [
    uniqueIndex("payment_customers_provider_cus_unique").on(
      table.provider,
      table.providerCustomerId,
    ),
    uniqueIndex("payment_customers_org_provider_unique").on(
      table.organizationId,
      table.provider,
    ),
    index("payment_customers_org_idx").on(table.organizationId),
  ],
);
export const paymentMethods = pgTable(
  "payment_methods",
  {
    ...
    organizationId: text("organization_id")
      ...
      .references(() => organizations.id, { onDelete: "restrict" }),
    provider: text().notNull(),
    externalPaymentMethodId: text("external_payment_method_id").notNull(),
    type: text().notNull(),
    display: jsonb().notNull().default({}),
    isDefault: boolean("is_default").notNull().default(false),
    status: text().notNull().default("active"),
    ...timestamps,
  },
  (table) => [
    uniqueIndex("payment_methods_provider_external_unique").on(
      table.provider,
      table.externalPaymentMethodId,
    ),
...
export const checkoutSessions = pgTable(
  "checkout_sessions",
  {
    ...
    successReturnUrl: text("success_return_url").notNull(),
    cancelReturnUrl: text("cancel_return_url").notNull(),
  ...
  (table) => [
    uniqueIndex("checkout_sessions_org_idempotency_unique").on(
      table.organizationId,
      table.idempotencyKey,
    ),
    index("checkout_sessions_provider_session_idx").on(
      table.provider,
      table.providerSessionId,
    ),
    index("checkout_sessions_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
  ],
...
export const paymentAttempts = pgTable(
  ...
  (table) => [
    uniqueIndex("payment_attempts_payment_number_unique").on(
      table.paymentId,
      table.attemptNumber,
    ),
    index("payment_attempts_payment_idx").on(table.paymentId),
  ],
);
export const paymentEvents = pgTable(
  ...
  (table) => [
    uniqueIndex("payment_events_provider_external_unique").on(
      table.provider,
      table.externalEventId,
    ),
    index("payment_events_status_idx").on(table.status, table.receivedAt),
  ],
);
export const refunds = pgTable(
  ...
  (table) => [
    uniqueIndex("refunds_provider_external_unique").on(
      table.provider,
      table.externalRefundId,
    ),
    index("refunds_payment_idx").on(table.paymentId),
    check("refunds_positive", sql`${table.amountMinor} > 0`),
  ],
);
...
export const invoices = pgTable(
  "invoices",
  {
    ...
    organizationId: text("organization_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "restrict" }),
    subscriptionPeriodId: text("subscription_period_id").references(
  ...
  (table) => [
    ...
    check(
      "invoices_nonnegative",
      sql`${table.subtotalMinor} >= 0 AND ${table.discountMinor} >= 0 AND ${table.taxMinor} >= 0 AND ${table.totalMinor} >= 0 AND ${table.amountPaidMinor} >= 0`,
    ),
  ],
);
export const invoiceLines = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("invoice_lines_source_unique").on(
      table.invoiceId,
      table.sourceType,
      table.sourceId,
    ),
...
export const taxRates = pgTable(
  ...
  (table) => [
    uniqueIndex("tax_rates_version_scope_unique").on(
      table.country,
      table.region,
      table.category,
      table.version,
...
export const exchangeRates = pgTable(
  "exchange_rates",
  {
    ...
    numerator: bigint({ mode: "bigint" }).notNull(),
    denominator: bigint({ mode: "bigint" }).notNull(),
    source: text().notNull(),
  ...
  (table) => [
    uniqueIndex("exchange_rates_observation_unique").on(
      table.baseCurrency,
      table.quoteCurrency,
      table.source,
      table.observedAt,
...
export const ledgerAccounts = pgTable(
  ...
  (table) => [
    uniqueIndex("ledger_accounts_scope_code_unique").on(
      table.organizationId,
      table.code,
    ),
  ],
...
export const ledgerTransactions = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("ledger_transactions_reference_unique").on(
      table.type,
      table.referenceType,
      table.referenceId,
    ),
...
export const ledgerEntries = pgTable(
  ...
  (table) => [
    ...
    check("ledger_entries_positive", sql`${table.amountMinor} > 0`),
    check(
...
export const usageSettlements = pgTable(
  "usage_settlements",
  {
    id: text().primaryKey(),
    requestId: text("request_id")
    ...
    organizationId: text("organization_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "restrict" }),
    providerCostMinor: bigint("provider_cost_minor", {
      mode: "bigint",
    }).notNull(),
    customerChargeMinor: bigint("customer_charge_minor", {
      mode: "bigint",
    }).notNull(),
    creditsConsumed: bigint("credits_consumed", { mode: "bigint" }).notNull(),
    currency: text().notNull(),
    pricingVersion: integer("pricing_version").notNull(),
    conversionVersion: integer("conversion_version").notNull(),
  ...
  (table) => [
    uniqueIndex("usage_settlements_request_unique").on(table.requestId),
    uniqueIndex("usage_settlements_usage_unique").on(table.usageRecordId),
    check(
      "usage_settlements_nonnegative",
      sql`${table.providerCostMinor} >= 0 AND ${table.customerChargeMinor} >= 0 AND ${table.creditsConsumed} >= 0`,
    ),
  ],
);
export const providerPriceSchedules = pgTable(
  "provider_price_schedules",
  {
    id: text().primaryKey(),
    providerId: text("provider_id")
      .notNull()
      .references(() => providers.id, { onDelete: "restrict" }),
    providerRouteId: text("provider_route_id").references(
      () => modelProviderRoutes.id,
      { onDelete: "cascade" },
    ),
  ...
  (table) => [
    index("provider_price_schedules_provider_idx").on(
      table.providerId,
      table.status,
    ),
    index("provider_price_schedules_effective_idx").on(
      table.effectiveFrom,
      table.effectiveTo,
    ),
  ],
...
export const providerRates = pgTable(
  "provider_rates",
  {
    ...
    scheduleId: text("schedule_id")
      .notNull()
      .references(() => providerPriceSchedules.id, { onDelete: "cascade" }),
    usageType: text("usage_type").notNull(),
    unit: text().notNull().default("token"),
    price: text().notNull(),
    perUnits: bigint("per_units", { mode: "bigint" })
      .notNull()
      .default(sql`1000000`),
...
export const providerCostRecords = pgTable(
  "provider_cost_records",
  {
    ...
    priceVersionSet: jsonb("price_version_set")
      .notNull()
      .default(sql`'[]'::jsonb`),
  ...
  (table) => [
    ...
    index("provider_cost_records_tenant_idx").on(
      table.organizationId,
      table.workspaceId,
      table.createdAt,
    ),
...
export const providerCostLines = pgTable(
  ...
  {
    id: text().primaryKey(),
    costRecordId: text("cost_record_id")
      .notNull()
      .references(() => providerCostRecords.id, { onDelete: "cascade" }),
    requestId: text("request_id")
      .notNull()
      .references(() => gatewayRequests.id, { onDelete: "restrict" }),
    attemptId: text("attempt_id").references(() => providerAttempts.id, {
    ...
    priceScheduleId: text("price_schedule_id")
      .notNull()
      .references(() => providerPriceSchedules.id, { onDelete: "restrict" }),
...
export const customerPricingPolicies = pgTable(
  "customer_pricing_policies",
  {
    ...
    cachePricingMode: text("cache_pricing_mode")
      .notNull()
      .default("discount_percentage"),
    cacheDiscountPercentage: text("cache_discount_percentage"),
    retryOverheadPolicy: text("retry_overhead_policy")
      .notNull()
      .default("absorbed_by_growx"),
    markupBasisPoints: bigint("markup_basis_points", { mode: "bigint" }),
    markupMultiplier: text("markup_multiplier"),
    fixedFee: text("fixed_fee"),
    minimumMarginBasisPoints: bigint("minimum_margin_basis_points", {
      mode: "bigint",
    }),
    ...timestamps,
  },
  (table) => [
    index("customer_pricing_policies_scope_idx").on(
      table.scopeType,
      table.scopeId,
      table.status,
    ),
...
export const customerRateSchedules = pgTable(
  "customer_rate_schedules",
  {
    id: text().primaryKey(),
    pricingPolicyId: text("pricing_policy_id")
      .notNull()
      .references(() => customerPricingPolicies.id, { onDelete: "cascade" }),
    canonicalModelId: text("canonical_model_id").references(
      () => canonicalModels.id,
      { onDelete: "cascade" },
    ),
  ...
  (table) => [
    ...
    index("customer_rate_schedules_model_idx").on(
...
export const customerRates = pgTable(
  "customer_rates",
  {
    ...
    scheduleId: text("schedule_id")
      .notNull()
      .references(() => customerRateSchedules.id, { onDelete: "cascade" }),
    usageType: text("usage_type").notNull(),
    unit: text().notNull().default("token"),
    price: text().notNull(),
    perUnits: bigint("per_units", { mode: "bigint" })
  ...
  (table) => [index("customer_rates_schedule_idx").on(table.scheduleId)],
);

export const customerPriceRecords = pgTable(
  "customer_price_records",
  {
    id: text().primaryKey(),
    requestId: text("request_id")
      .notNull()
    ...
    pricingPolicyId: text("pricing_policy_id")
      .notNull()
      .references(() => customerPricingPolicies.id, { onDelete: "restrict" }),
    pricingPolicyVersion: integer("pricing_policy_version")
      .notNull()
      .default(1),
    currency: text().notNull().default("USD"),
    subtotal: text().notNull().default("0"),
...
export const customerPriceLines = pgTable(
  "customer_price_lines",
  {
    id: text().primaryKey(),
    priceRecordId: text("price_record_id")
      .notNull()
      .references(() => customerPriceRecords.id, { onDelete: "cascade" }),
    usageType: text("usage_type").notNull(),
    quantity: bigint("quantity", { mode: "bigint" }).notNull(),
    unit: text().notNull().default("token"),
    rate: text().notNull(),
  ...
  (table) => [index("customer_price_lines_record_idx").on(table.priceRecordId)],
);

export const pricingAdjustments = pgTable(
  "pricing_adjustments",
  {
    id: text().primaryKey(),
    requestId: text("request_id")
  ...
  (table) => [
    ...
    index("pricing_adjustments_tenant_idx").on(
      table.organizationId,
      table.workspaceId,
      table.createdAt,
    ),
...
export const reconciliationRuns = pgTable(
  "reconciliation_runs",
  {
    ...
    periodEnd: timestamp("period_end", { withTimezone: true }).notNull(),
    status: text().notNull(),
    examined: integer().notNull().default(0),
    mismatches: integer().notNull().default(0),
    startedAt: timestamp("started_at", { withTimezone: true }).notNull(),
    completedAt: timestamp("completed_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    index("reconciliation_runs_domain_period_idx").on(
      table.domain,
      table.periodStart,
      table.periodEnd,
    ),
...
export const webhookEndpoints = pgTable(
  ...
  (table) => [
    index("webhook_endpoints_scope_idx").on(
      table.organizationId,
      table.workspaceId,
      table.status,
    ),
...
export const webhookSubscriptions = pgTable(
  ...
  (table) => [
    primaryKey({ columns: [table.webhookEndpointId, table.eventType] }),
  ],
);
export const webhookDeliveries = pgTable(
  ...
  (table) => [
    ...
    index("webhook_delivery_queue_idx").on(table.status, table.nextAttemptAt),
    index("webhook_delivery_scope_idx").on(
      table.organizationId,
      table.workspaceId,
      table.createdAt,
    ),
...
export const webhookDeliveryAttempts = pgTable(
  "webhook_delivery_attempts",
  {
    ...
    responseBodyRedacted: text("response_body_redacted"),
    latencyMs: integer("latency_ms"),
    failureCode: text("failure_code"),
    attemptedAt: timestamp("attempted_at", { withTimezone: true }).notNull(),
  },
  (table) => [
    uniqueIndex("webhook_attempt_unique").on(table.deliveryId, table.attempt),
  ],
);
...
export const notifications = pgTable(
  ...
  (table) => [
    index("notifications_user_state_idx").on(
      table.userId,
      table.readAt,
      table.createdAt,
    ),
...
export const notificationPreferences = pgTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.userId, table.organizationId, table.category],
    }),
  ],
);
export const alertRules = pgTable(
  "alert_rules",
  {
    ...
    metric: text().notNull(),
    operator: text().notNull(),
    threshold: bigint({ mode: "bigint" }).notNull(),
    period: text().notNull(),
    channels: jsonb().notNull(),
    status: text().notNull().default("active"),
    ...timestamps,
  },
  (table) => [
    index("alert_rules_scope_idx").on(
      table.organizationId,
      table.workspaceId,
      table.status,
    ),
...
export const serviceAccounts = pgTable(
  ...
  (table) => [
    uniqueIndex("service_accounts_scope_name_unique").on(
      table.organizationId,
      table.workspaceId,
      table.name,
    ),
...
export const serviceAccountCredentials = pgTable(
  ...
  (table) => [
    uniqueIndex("service_account_credentials_prefix_unique").on(table.prefix),
    index("service_account_credentials_account_idx").on(table.serviceAccountId),
  ],
);
...
export const exportJobs = pgTable(
  ...
  (table) => [
    index("exports_scope_status_idx").on(
      table.organizationId,
      table.workspaceId,
      table.status,
    ),
...
export const developerOnboarding = pgTable(
  "developer_onboarding",
  {
    ...
    workspaceId: text("workspace_id")
      .notNull()
      .references(() => workspaces.id, { onDelete: "cascade" }),
    firstApiKeyAt: timestamp("first_api_key_at", { withTimezone: true }),
    firstGatewayRequestAt: timestamp("first_gateway_request_at", {
      withTimezone: true,
    }),
    firstSuccessfulRequestAt: timestamp("first_successful_request_at", {
      withTimezone: true,
    }),
    firstPlaygroundRequestAt: timestamp("first_playground_request_at", {
      withTimezone: true,
    }),
    billingConfiguredAt: timestamp("billing_configured_at", {
      withTimezone: true,
    }),
    updatedAt: timestamp("updated_at", { withTimezone: true })
      .notNull()
  ...
  (table) => [
    primaryKey({ columns: [table.organizationId, table.workspaceId] }),
  ],
);
export const userPreferences = pgTable("user_preferences", {
  userId: text("user_id")
    .primaryKey()
    .references(() => users.id, { onDelete: "cascade" }),
  preferredOrganizationId: text("preferred_organization_id").references(
    () => organizations.id,
    { onDelete: "set null" },
  ),
  preferredWorkspaceId: text("preferred_workspace_id").references(
    () => workspaces.id,
    { onDelete: "set null" },
  ),
  preferredEnvironmentId: text("preferred_environment_id").references(
    () => environments.id,
    { onDelete: "set null" },
  ),
  ...
  timezone: text().notNull().default("UTC"),
  locale: text().notNull().default("en"),
  defaultPlaygroundModel: text("default_playground_model"),
  updatedAt: timestamp("updated_at", { withTimezone: true })
    .notNull()
    .defaultNow(),
});
...
export const incidents = pgTable(
  ...
  (table) => [
    index("incidents_status_started_idx").on(table.status, table.startedAt),
  ],
);
export const incidentUpdates = pgTable(
  ...
  (table) => [
    index("incident_updates_incident_time_idx").on(
      table.incidentId,
      table.createdAt,
    ),
  ],
...
export const privilegedSessions = pgTable(
  "privileged_sessions",
  {
    id: text().primaryKey(),
    operatorId: text("operator_id").notNull(),
    authenticationStrength: text("authentication_strength").notNull(),
    reason: text().notNull(),
  ...
  (table) => [
    index("privileged_sessions_operator_expiry_idx").on(
      table.operatorId,
      table.expiresAt,
    ),
  ],
);
export const privilegedSessionCapabilities = pgTable(
  "privileged_session_capabilities",
  {
    sessionId: text("session_id")
      .notNull()
      .references(() => privilegedSessions.id, { onDelete: "cascade" }),
    capability: text().notNull(),
  },
  (table) => [primaryKey({ columns: [table.sessionId, table.capability] })],
);
export const privilegedAuditEvents = pgTable(
  "privileged_audit_events",
  {
    ...
    sessionId: text("session_id")
      .notNull()
      .references(() => privilegedSessions.id, { onDelete: "restrict" }),
    operatorId: text("operator_id").notNull(),
    action: text().notNull(),
  ...
  (table) => [
    index("privileged_audit_operator_time_idx").on(
      table.operatorId,
      table.createdAt,
    ),
    index("privileged_audit_resource_idx").on(
      table.resourceType,
      table.resourceId,
    ),
  ],
...
export const walletBalances = pgTable("wallet_balances", {
  ...
  available: numeric("available", { precision: 36, scale: 18 })
    .notNull()
    .default("0"),
  reserved: numeric("reserved", { precision: 36, scale: 18 })
    .notNull()
    .default("0"),
...
export const walletLedgerEntries = pgTable(
  ...
  (table) => [
    uniqueIndex("wallet_ledger_wallet_sequence_unique").on(
      table.walletId,
      table.sequence,
    ),
    index("wallet_ledger_wallet_time_idx").on(table.walletId, table.createdAt),
    index("wallet_ledger_reference_idx").on(
      table.referenceType,
      table.referenceId,
    ),
  ],
...
export const creditLots = pgTable(
  "credit_lots",
  {
    ...
    reservedAmount: numeric("reserved_amount", { precision: 36, scale: 18 })
      .notNull()
      .default("0"),
    sourceType: text("source_type").notNull(),
    sourceId: text("source_id").notNull(),
...
export const reservationAllocations = pgTable(
  ...
  (table) => [index("reservation_allocations_res_idx").on(table.reservationId)],
);
...
export const workspaceBudgets = pgTable(
  "workspace_budgets",
  {
    ...
    spentInPeriod: numeric("spent_in_period", { precision: 36, scale: 18 })
      .notNull()
      .default("0"),
    reservedInPeriod: numeric("reserved_in_period", {
      precision: 36,
      scale: 18,
    })
      .notNull()
...
export const billingAuthorizationRecords = pgTable(
  "billing_authorization_records",
  {
    id: text().primaryKey(),
    ...
    requiredReservation: numeric("required_reservation", {
      precision: 36,
      scale: 18,
    }).notNull(),
    availableAtDecision: numeric("available_at_decision", {
      precision: 36,
      scale: 18,
    }),
    currency: text().notNull().default("USD"),
  ...
  (table) => [
    index("billing_auth_records_org_ws_idx").on(
      table.organizationId,
      table.workspaceId,
      table.createdAt,
    ),
...
export const settlementShortfalls = pgTable(
  "settlement_shortfalls",
  {
    ...
    finalCustomerPrice: numeric("final_customer_price", {
      precision: 36,
      scale: 18,
    }).notNull(),
    shortfallAmount: numeric("shortfall_amount", {
      precision: 36,
      scale: 18,
    }).notNull(),
    status: text().notNull().default("open"),
  ...
  (table) => [
    index("settlement_shortfalls_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
  ],
...
export const walletAdjustmentLogs = pgTable(
  ...
  (table) => [
    index("wallet_adjustments_org_idx").on(
      table.organizationId,
      table.createdAt,
    ),
  ],
...
export const planVersions = pgTable(
  "plan_versions",
  {
    ...
    basePriceAmount: numeric("base_price_amount", { precision: 36, scale: 18 })
      .notNull()
      .default("0"),
    currency: text().notNull().default("USD"),
    creditGrantAmount: numeric("credit_grant_amount", {
      precision: 36,
      scale: 18,
    })
      .notNull()
  ...
  (table) => [
    uniqueIndex("plan_versions_plan_version_unique").on(
      table.planId,
      table.version,
    ),
...
export const planEntitlements = pgTable(
  ...
  (table) => [
    uniqueIndex("plan_entitlements_version_key_unique").on(
      table.planVersionId,
      table.key,
    ),
  ],
...
export const planModelAccessRules = pgTable(
  "plan_model_access_rules",
  {
    id: text().primaryKey(),
    planVersionId: text("plan_version_id")
      .notNull()
...
export const planLimits = pgTable(
  ...
  (table) => [
    uniqueIndex("plan_limits_version_key_unique").on(
      table.planVersionId,
      table.key,
    ),
  ],
...
export const organizationSubscriptions = pgTable(
  "organization_subscriptions",
  {
    ...
    fundingMode: subscriptionFundingMode("funding_mode")
      .notNull()
      .default("manual"),
    metadata: jsonb().notNull().default({}),
    ...timestamps,
  ...
  (table) => [
    index("org_subscriptions_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
    index("org_subscriptions_renewal_idx").on(
...
export const subscriptionPeriods = pgTable(
  "subscription_periods",
  {
    ...
    subscriptionId: text("subscription_id")
      .notNull()
      .references(() => organizationSubscriptions.id, { onDelete: "restrict" }),
  ...
  (table) => [
    uniqueIndex("subscription_periods_sub_number_unique").on(
...
export const entitlementOverrides = pgTable(
  ...
  (table) => [
    uniqueIndex("entitlement_overrides_org_key_unique").on(
      table.organizationId,
      table.key,
    ),
  ],
...
export const invoiceLegalEntitySnapshots = pgTable(
  "invoice_legal_entity_snapshots",
  {
    id: text().primaryKey(),
...
export const invoiceBillingProfileSnapshots = pgTable(
  "invoice_billing_profile_snapshots",
  {
    id: text().primaryKey(),
...
export const taxRules = pgTable(
  ...
  (table) => [
    index("tax_rules_regime_jurisdiction_idx").on(
      table.regime,
      table.jurisdiction,
      table.status,
    ),
...
export const invoiceSequences = pgTable(
  ...
  (table) => [
    uniqueIndex("invoice_sequences_entity_fy_unique").on(
      table.legalEntityId,
      table.fiscalYear,
    ),
  ],
...
export const invoiceTaxLines = pgTable(
  ...
  (table) => [index("invoice_tax_lines_invoice_idx").on(table.invoiceId)],
);
...
export const invoicePaymentAllocations = pgTable(
  ...
  (table) => [
    uniqueIndex("invoice_payment_allocations_inv_pay_unique").on(
      table.invoiceId,
      table.paymentId,
    ),
    uniqueIndex("invoice_payment_allocations_idempotency_unique").on(
      table.idempotencyKey,
    ),
  ],
);
...
export const creditNotes = pgTable(
  "credit_notes",
  {
    ...
    taxTotal: numeric("tax_total", { precision: 36, scale: 18 })
      .notNull()
      .default("0"),
    total: numeric({ precision: 36, scale: 18 }).notNull(),
    amountAllocated: numeric("amount_allocated", { precision: 36, scale: 18 })
      .notNull()
      .default("0"),
    reason: text().notNull(),
...
export const creditNoteLines = pgTable(
  "credit_note_lines",
  {
    ...
    lineNumber: integer("line_number").notNull(),
    description: text().notNull(),
    quantity: bigint({ mode: "bigint" })
      .notNull()
      .default(sql`1`),
    unitPrice: numeric("unit_price", { precision: 36, scale: 18 }).notNull(),
    subtotal: numeric({ precision: 36, scale: 18 }).notNull(),
    taxAmount: numeric("tax_amount", { precision: 36, scale: 18 })
      .notNull()
      .default("0"),
    total: numeric({ precision: 36, scale: 18 }).notNull(),
  },
...
export const invoiceDocuments = pgTable(
  ...
  (table) => [
    uniqueIndex("invoice_documents_invoice_version_unique").on(
      table.invoiceId,
      table.version,
    ),
...
export const outboundWebhookEvents = pgTable(
  ...
  (table) => [
    uniqueIndex("outbound_events_source_unique").on(
      table.sourceEventId,
      table.eventType,
      table.eventVersion,
    ),
    index("outbound_events_org_created_idx").on(
      table.organizationId,
      table.createdAt,
    ),
  ],
...
export const webhookSigningSecrets = pgTable(
  ...
  (table) => [
    index("webhook_secrets_endpoint_status_idx").on(
      table.endpointId,
      table.status,
    ),
  ],
...
export const webhookReplayJobs = pgTable(
  "webhook_replay_jobs",
  {
    id: text().primaryKey(),
    organizationId: text("organization_id")
  ...
  (table) => [
    index("webhook_replay_jobs_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
  ],
...
export const securitySignals = pgTable(
  "security_signals",
  {
    id: text().primaryKey(),
  ...
  (table) => [
    index("security_signals_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
...
export const securityCases = pgTable(
  ...
  (table) => [
    index("security_cases_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
  ],
...
export const notificationIntents = pgTable(
  ...
  (table) => [
    uniqueIndex("notification_intents_source_type_unique").on(
      table.sourceEventId,
      table.type,
    ),
    index("notification_intents_org_created_idx").on(
      table.organizationId,
      table.createdAt,
    ),
  ],
...
export const notificationDeliveries = pgTable(
  ...
  (table) => [
    ...
    index("notification_deliveries_priority_sched_idx").on(
      table.priority,
      table.scheduledAt,
    ),
    index("notification_deliveries_recipient_idx").on(
      table.recipientSnapshot,
      table.createdAt,
    ),
  ],
);
...
export const notificationDeliveryAttempts = pgTable(
  ...
  (table) => [
    index("notification_attempts_delivery_idx").on(
      table.deliveryId,
      table.attemptNumber,
    ),
  ],
...
export const organizationNotificationSettings = pgTable(
  "organization_notification_settings",
  {
    ...
    securityAlertsEnabled: boolean("security_alerts_enabled")
      .notNull()
      .default(true),
    billingAlertsEnabled: boolean("billing_alerts_enabled")
      .notNull()
      .default(true),
...
export const notificationEscalations = pgTable(
  ...
  (table) => [
    index("notification_escalations_status_time_idx").on(
      table.status,
      table.nextEscalationAt,
    ),
  ],
);
...
export const semanticCacheEntries = pgTable(
  ...
  (table) => [
    index("semantic_cache_org_ws_ns_idx").on(
      table.organizationId,
      table.workspaceId,
      table.namespaceHash,
    ),
...
export const semanticEmbeddings = pgTable(
  ...
  (table) => [
    uniqueIndex("semantic_embeddings_org_hash_unique").on(
      table.organizationId,
      table.textHash,
    ),
  ],
...
export const files = pgTable(
  "files",
  {
    ...
    etag: text(),
    encryptionState: text("encryption_state")
      .notNull()
      .default("provider_encrypted"),
    safetyState: fileSafetyState("safety_state")
      .notNull()
      .default("not_scanned"),
    metadata: jsonb().notNull().default({}),
...
export const fileUploadSessions = pgTable(
  ...
  (table) => [
    ...
    index("file_upload_sessions_expiry_status_idx").on(
      table.expiresAt,
      table.status,
    ),
  ],
...
export const fileProviderReferences = pgTable(
  "file_provider_references",
  {
    id: text().primaryKey(),
    fileId: text("file_id")
      .notNull()
    ...
    providerStatus: fileProviderReferenceStatus("provider_status")
      .notNull()
      .default("ready"),
    expiresAt: timestamp("expires_at", { withTimezone: true }),
    createdAt: timestamp("created_at", { withTimezone: true })
  ...
  (table) => [
    ...
    uniqueIndex("file_provider_refs_unique").on(
      table.fileId,
      table.providerId,
...
export const fileUsageReferences = pgTable(
  ...
  (table) => [
    ...
    index("file_usage_refs_type_id_idx").on(
      table.referenceType,
      table.referenceId,
    ),
  ],
...
export const fileStorageReservations = pgTable(
  "file_storage_reservations",
  {
    id: text().primaryKey(),
    fileId: text("file_id")
      .notNull()
...
export const fileRetentionPolicies = pgTable(
  ...
  (table) => [
    index("file_retention_policies_org_purpose_idx").on(
      table.organizationId,
      table.purpose,
    ),
  ],
...
export const batchJobs = pgTable(
  "batch_jobs",
  {
    ...
    inputFileId: text("input_file_id").references(() => files.id, {
      onDelete: "set null",
    }),
    outputFileId: text("output_file_id").references(() => files.id, {
      onDelete: "set null",
    }),
    errorFileId: text("error_file_id").references(() => files.id, {
      onDelete: "set null",
    }),
  ...
  (table) => [
    index("batch_jobs_org_created_idx").on(
      table.organizationId,
      table.createdAt,
    ),
    ...
    index("batch_jobs_status_deadline_idx").on(
      table.status,
      table.executionDeadlineAt,
    ),
  ],
);
...
export const batchItems = pgTable(
  ...
  (table) => [
    uniqueIndex("batch_items_batch_custom_id_unique").on(
      table.batchId,
      table.customId,
    ),
    uniqueIndex("batch_items_batch_pos_unique").on(
      table.batchId,
      table.position,
    ),
...
export const batchItemAttempts = pgTable(
  "batch_item_attempts",
  {
    id: text().primaryKey(),
  ...
  (table) => [
    uniqueIndex("batch_attempts_item_attempt_unique").on(
      table.batchItemId,
      table.attemptNumber,
    ),
...
export const batchChunks = pgTable(
  ...
  (table) => [
    uniqueIndex("batch_chunks_batch_seq_unique").on(
      table.batchId,
      table.sequence,
    ),
...
export const batchLeases = pgTable(
  "batch_leases",
  {
    id: text().primaryKey(),
    resourceType: text("resource_type").notNull(),
  ...
  (table) => [
    uniqueIndex("batch_leases_type_res_unique").on(
      table.resourceType,
      table.resourceId,
    ),
...
export const batchExecutionReservations = pgTable(
  "batch_execution_reservations",
  {
    id: text().primaryKey(),
    batchId: text("batch_id")
      .notNull()
    ...
    organizationId: text("organization_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "cascade" }),
    reservedCreditsAmount: numeric("reserved_credits_amount", {
      precision: 36,
      scale: 18,
    }).notNull(),
    settledCreditsAmount: numeric("settled_credits_amount", {
      precision: 36,
      scale: 18,
    })
      .notNull()
...
export const batchIdempotencyRecords = pgTable(
  "batch_idempotency_records",
  {
    id: text().primaryKey(),
    organizationId: text("organization_id")
      .notNull()
  ...
  (table) => [
    uniqueIndex("batch_idem_org_key_unique").on(
      table.organizationId,
      table.idempotencyKey,
    ),
...
export const routeTrafficControls = pgTable(
  "route_traffic_controls",
  {
    id: text().primaryKey(),
    routeId: text("route_id")
      .notNull()
      .references(() => modelProviderRoutes.id, { onDelete: "cascade" }),
...
export const providerCredentialPools = pgTable(
  ...
  (table) => [
    index("provider_credential_pools_provider_idx").on(
      table.providerId,
      table.status,
    ),
  ],
...
export const routingDecisionsV2 = pgTable(
  "routing_decisions_v2",
  {
    ...
    selectedRouteId: text("selected_route_id")
      .notNull()
      .references(() => modelProviderRoutes.id, { onDelete: "restrict" }),
  ...
  (table) => [
    ...
    index("routing_decisions_v2_tenant_idx").on(
      table.organizationId,
      table.workspaceId,
      table.createdAt,
    ),
...
export const routingCandidateDecisions = pgTable(
  "routing_candidate_decisions",
  {
    id: text().primaryKey(),
    routingDecisionId: text("routing_decision_id")
      .notNull()
      .references(() => routingDecisionsV2.id, { onDelete: "cascade" }),
    routeId: text("route_id")
      .notNull()
      .references(() => modelProviderRoutes.id, { onDelete: "cascade" }),
    eligible: boolean().notNull(),
  ...
  (table) => [
    index("routing_candidate_decisions_decision_idx").on(
      table.routingDecisionId,
    ),
  ],
);

export const routingSnapshots = pgTable(
  "routing_snapshots",
  {
    id: text().primaryKey(),
    version: integer().notNull(),
...
export const providerCredentialVersions = pgTable(
  ...
  (table) => [
    uniqueIndex("provider_credential_versions_cred_ver_unique").on(
      table.credentialId,
      table.version,
    ),
    index("provider_credential_versions_secret_ref_idx").on(
...
export const providerCredentialPoolMembers = pgTable(
  "provider_credential_pool_members",
  {
    id: text().primaryKey(),
    poolId: text("pool_id")
      .notNull()
      .references(() => providerCredentialPools.id, { onDelete: "cascade" }),
  ...
  (table) => [
    uniqueIndex("provider_credential_pool_members_unique").on(
      table.poolId,
      table.providerAccountId,
      table.credentialId,
    ),
    index("provider_credential_pool_members_pool_idx").on(
      table.poolId,
      table.status,
    ),
  ],
...
export const providerAccountCapabilities = pgTable(
  ...
  (table) => [
    index("provider_account_capabilities_account_idx").on(
      table.providerAccountId,
      table.capability,
    ),
  ],
...
export const providerAccountLimits = pgTable(
  ...
  (table) => [
    index("provider_account_limits_account_idx").on(
      table.providerAccountId,
      table.limitType,
    ),
  ],
...
export const credentialValidations = pgTable(
  "credential_validations",
  {
    id: text().primaryKey(),
    credentialVersionId: text("credential_version_id")
      .notNull()
      .references(() => providerCredentialVersions.id, { onDelete: "cascade" }),
    status: text().notNull(),
    latencyMs: integer("latency_ms"),
  ...
  (table) => [
    index("credential_validations_version_idx").on(
...
export const promptDefinitions = pgTable(
  ...
  (table) => [
    ...
    index("prompt_definitions_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
    uniqueIndex("prompt_definitions_org_ws_key_unique").on(
      table.organizationId,
      table.workspaceId,
      table.key,
    ),
...
export const promptVersions = pgTable(
  ...
  (table) => [
    ...
    index("prompt_versions_prompt_hash_idx").on(
      table.promptId,
      table.contentHash,
    ),
  ],
...
export const promptReleases = pgTable(
  ...
  (table) => [
    index("prompt_releases_prompt_env_status_idx").on(
      table.promptId,
      table.environment,
      table.status,
    ),
...
export const promptReleaseHeads = pgTable(
  "prompt_release_heads",
  {
    id: text().primaryKey(),
    promptId: text("prompt_id")
      .notNull()
    ...
    activeReleaseId: text("active_release_id")
      .notNull()
      .references(() => promptReleases.id, { onDelete: "cascade" }),
  ...
  (table) => [
    uniqueIndex("prompt_release_heads_prompt_env_unique").on(
      table.promptId,
      table.environment,
    ),
  ],
...
export const promptExecutionReferences = pgTable(
  ...
  (table) => [
    ...
    index("prompt_execution_references_prompt_version_idx").on(
      table.promptId,
...
export const registeredTools = pgTable(
  "registered_tools",
  {
    id: text().primaryKey(),
    organizationId: text("organization_id").references(() => organizations.id, {
  ...
  (table) => [
    ...
    index("registered_tools_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
    uniqueIndex("registered_tools_org_ws_key_unique").on(
      table.organizationId,
      table.workspaceId,
      table.key,
    ),
...
export const registeredToolVersions = pgTable(
  "registered_tool_versions",
  {
    id: text().primaryKey(),
    toolId: text("tool_id")
      .notNull()
      .references(() => registeredTools.id, { onDelete: "cascade" }),
    version: integer().notNull(),
    description: text(),
  ...
  (table) => [
    uniqueIndex("registered_tool_versions_tool_version_unique").on(
      table.toolId,
      table.version,
    ),
    index("registered_tool_versions_tool_hash_idx").on(
      table.toolId,
      table.contentHash,
    ),
  ],
...
export const toolCallRecords = pgTable(
  "tool_call_records",
  {
    id: text().primaryKey(),
    requestId: text("request_id")
  ...
  (table) => [
    ...
    index("tool_call_records_org_created_idx").on(
      table.organizationId,
      table.createdAt,
    ),
  ],
...
export const toolContinuationStates = pgTable(
  "tool_continuation_states",
  {
    id: text().primaryKey(),
    requestId: text("request_id")
      .notNull()
  ...
  (table) => [
    ...
    index("tool_continuation_states_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
...
export const toolExecutionRecords = pgTable(
  "tool_execution_records",
  {
    id: text().primaryKey(),
    toolCallId: text("tool_call_id")
      .notNull()
      .references(() => toolCallRecords.id, { onDelete: "cascade" }),
    organizationId: text("organization_id")
      .notNull()
      .references(() => organizations.id, { onDelete: "cascade" }),
  ...
  (table) => [
    ...
    index("tool_execution_records_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
  ],
...
export const responseSchemaStatus = pgEnum("response_schema_status", [
  "active",
  "disabled",
  "archived",
]);
export const responseSchemaVisibility = pgEnum("response_schema_visibility", [
  "organization",
  "workspace",
  "internal",
]);

export const responseSchemas = pgTable(
  "response_schemas",
  {
    id: text().primaryKey(),
    organizationId: text("organization_id")
  ...
  (table) => [
    uniqueIndex("response_schemas_org_key_idx").on(
      table.organizationId,
      table.key,
    ),
    index("response_schemas_org_status_idx").on(
      table.organizationId,
      table.status,
    ),
  ],
...
export const responseSchemaVersions = pgTable(
  "response_schema_versions",
  {
    id: text().primaryKey(),
    schemaId: text("schema_id")
      .notNull()
      .references(() => responseSchemas.id, { onDelete: "cascade" }),
    version: integer().notNull(),
  ...
  (table) => [
    uniqueIndex("response_schema_versions_sid_ver_idx").on(
      table.schemaId,
      table.version,
    ),
