...
describe('Embedding Vector Space Compatibility', () => {
  it('considers routes with identical canonicalModelId compatible', () => {
    ...
    expect(EmbeddingCompatibilityManager.isCompatible(route1, route2)).toBe(true);
    expect(() => EmbeddingCompatibilityManager.assertCompatible(route1, route2)).not.toThrow();
  });
  ...
  it('rejects fallback across different canonical models without explicit compatibility group', () => {
    ...
    expect(EmbeddingCompatibilityManager.isCompatible(route1, route2)).toBe(false);
    expect(() => EmbeddingCompatibilityManager.assertCompatible(route1, route2)).toThrow(
      EmbeddingVectorSpaceIncompatibleError
    );
  });
});
