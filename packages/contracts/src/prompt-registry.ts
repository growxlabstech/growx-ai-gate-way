...
export const promptVisibilitySchema = z.enum(["workspace", "organization", "internal"]);
export type PromptVisibility = z.infer<typeof promptVisibilitySchema>;

export const templateFormatSchema = z.enum(["mustache", "handlebars_safe", "template_literal"]);
export type TemplateFormat = z.infer<typeof templateFormatSchema>;

export const promptVariableTypeSchema = z.enum(["string", "number", "boolean", "array", "object"]);
export type PromptVariableType = z.infer<typeof promptVariableTypeSchema>;

export const promptReleaseEnvironmentSchema = z.enum(["development", "staging", "production"]);
export type PromptReleaseEnvironment = z.infer<typeof promptReleaseEnvironmentSchema>;

export const promptReleaseStatusSchema = z.enum(["active", "retired", "rolled_back"]);
...
export const promptDefinitionSchema = z.object({
  ...
  key: z.string().min(2).max(128).regex(/^[a-zA-Z0-9._-]+$/),
...
export type PromptExecutionReference = z.infer<typeof promptExecutionReferenceSchema>;
...
export type CreatePromptVersionRequest = z.input<typeof createPromptVersionRequestSchema>;

export const createPromptReleaseRequestSchema = z.object({
  promptVersionId: z.string().min(1),
  environment: promptReleaseEnvironmentSchema.default("production"),
  notes: z.string().max(1024).optional(),
}).strict();
export type CreatePromptReleaseRequest = z.input<typeof createPromptReleaseRequestSchema>;

export const rollbackPromptReleaseRequestSchema = z.object({
  environment: promptReleaseEnvironmentSchema.default("production"),
  targetVersionId: z.string().optional(), // If omitted, rolls back to the previous active release
  reason: z.string().max(500).optional(),
}).strict();
export type RollbackPromptReleaseRequest = z.input<typeof rollbackPromptReleaseRequestSchema>;
...
export const validatePromptRequestSchema = z.object({
  messages: z.array(promptMessageTemplateSchema).min(1).max(64).optional(),
  template: z.string().min(1).max(200_000).optional(),
  templateFormat: templateFormatSchema.default("mustache"),
  variableSchema: z.array(promptVariableDefinitionSchema).default([]),
...
export type PromptExecutionBinding = z.infer<typeof promptExecutionBindingSchema>;
