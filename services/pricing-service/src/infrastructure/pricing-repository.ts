...
export interface PricingRepository {
  ...
  getProviderSchedule(id: string): Promise<ProviderScheduleWithRates | undefined>;
  listProviderSchedules(filter?: { providerId?: string; status?: string }): Promise<ProviderScheduleWithRates[]>;
  ...
  listCustomerPolicies(filter?: { scopeType?: string; scopeId?: string; status?: string }): Promise<CustomerPolicyWithRates[]>;

  // Cost records
  saveProviderCostRecord(record: ProviderCostRecord): Promise<void>;
  getProviderCostRecord(requestId: string): Promise<ProviderCostRecord | undefined>;

  // Customer price records
  saveCustomerPriceRecord(record: CustomerPriceRecord): Promise<void>;
  getCustomerPriceRecord(requestId: string): Promise<CustomerPriceRecord | undefined>;
...
export class InMemoryPricingRepository implements PricingRepository {
  private readonly providerSchedules: Map<string, ProviderScheduleWithRates> = new Map();
  private readonly customerPolicies: Map<string, CustomerPolicyWithRates> = new Map();
  private readonly providerCostRecords: Map<string, ProviderCostRecord> = new Map();
  private readonly customerPriceRecords: Map<string, CustomerPriceRecord> = new Map();
  private readonly adjustments: PricingAdjustmentRecord[] = [];

  public async saveProviderSchedule(schedule: ProviderScheduleWithRates): Promise<void> {
  ...
  public async listProviderSchedules(filter?: { providerId?: string; status?: string }): Promise<ProviderScheduleWithRates[]> {
    ...
    if (filter?.providerId) {
      list = list.filter((s) => s.schedule.providerId.toLowerCase() === filter.providerId!.toLowerCase());
    }
    if (filter?.status) {
  ...
  public async saveCustomerPolicy(policy: CustomerPolicyWithRates): Promise<void> {
  ...
  public async saveProviderCostRecord(record: ProviderCostRecord): Promise<void> {
    this.providerCostRecords.set(record.requestId, record);
  }

  public async getProviderCostRecord(requestId: string): Promise<ProviderCostRecord | undefined> {
    return this.providerCostRecords.get(requestId);
  }

  public async saveCustomerPriceRecord(record: CustomerPriceRecord): Promise<void> {
    this.customerPriceRecords.set(record.requestId, record);
  }

  public async getCustomerPriceRecord(requestId: string): Promise<CustomerPriceRecord | undefined> {
    return this.customerPriceRecords.get(requestId);
  }

  public async savePricingAdjustment(adjustment: PricingAdjustmentRecord): Promise<void> {
    this.adjustments.push(adjustment);
  }

  public async listPricingAdjustments(requestId: string): Promise<PricingAdjustmentRecord[]> {
