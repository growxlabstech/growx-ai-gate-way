...
export function AuthCard({
  ...
  const [flow, setFlow] = useState<Flow>(
    mode === "verify-email" ? "verification" : "form",
  );
  ...
  async function submitEmail(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isRecovery) {
        setError(
          "Password recovery is unavailable from the passwordless sign-in experience.",
        );
        return;
      }
  ...
  async function verifyOtp(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const resolution = await fetch("/api/auth/resolve", {
        method: "POST",
        credentials: "include",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ returnTo: returnTo ?? null }),
      });
      const payload = (await resolution.json().catch(() => null)) as {
        destination?: unknown;
      } | null;
      if (!resolution.ok || typeof payload?.destination !== "string")
        throw new AuthRequestError(
          "Your session was created, but workspace context is temporarily unavailable. Try again.",
          null,
          false,
          null,
        );
  ...
  async function social(provider: Provider) {
    ...
    try {
      ...
      if (!payload || typeof payload.url !== "string")
        throw new Error("This provider is not configured right now.");
  ...
  const subtitle =
    flow === "verification"
      ? "Enter the code sent to continue."
      : "Use your email to sign in or create an account.";

  return (
    <AuthShell artwork={<AuthArtwork />}>
      <Link className="auth-brand" href="/" aria-label="GrowX AI home">
        ...
        <div className="auth-state" key={flow}>
          ...
          {flow === "verification" ? (
            <form
              ...
              aria-labelledby="verification-email"
              onSubmit={verifyOtp}
              noValidate
            >
              ...
              <OtpInput
                ...
                onChange={(value) => {
                  setOtp(value);
                  clearStatus();
                  if (value.length === 6 && !busy && !verificationLocked)
                    requestAnimationFrame(() =>
                      verificationFormRef.current?.requestSubmit(),
                    );
                }}
                disabled={busy || verificationLocked}
            ...
            <>
              <form className="auth-form" onSubmit={submitEmail} noValidate>
                ...
                <AuthStatus
                  ...
                  requestId={requestId}
                />
                <AuthPrimaryAction busy={busy} busyLabel="Sending code…">
              ...
              {!isRecovery ? (
                <p className="auth-switch">
                  New to GrowX? Continuing with email creates your account.
                </p>
              ) : null}
            </>
          )}
