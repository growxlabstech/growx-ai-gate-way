...
export async function AppShell({ organizationSlug, workspaceSlug, title, description, action, children }: ShellProps) {
  const result = await loadTenantContext();
  if (result.status === "unauthenticated") redirect(`/sign-in?returnTo=/${organizationSlug}${workspaceSlug ? `/${workspaceSlug}` : ""}`);
  if (result.status === "empty") redirect("/onboarding");
  if (result.status === "error") return <main className="shell-failure"><section className="state" role="alert"><div><h1>Workspace context unavailable</h1><p>GrowX could not safely load your organization and workspace access. Previous tenant data has not been shown.</p><a className="retry-link" href="">Retry</a></div></section></main>;

  const activeOrganization = result.context.organizations.find((organization) => organization.organizationSlug === organizationSlug && organization.status === "active");
  if (!activeOrganization) notFound();
  const availableWorkspaces = workspacesForOrganization(result.context, activeOrganization.organizationId);
  const activeWorkspace = workspaceSlug ? availableWorkspaces.find((workspace) => workspace.workspaceSlug === workspaceSlug) : undefined;
  if (workspaceSlug && !activeWorkspace) notFound();

  return <ConsoleShell context={result.context} activeOrganization={activeOrganization} activeWorkspace={activeWorkspace} title={title} description={description} action={action}>{children}</ConsoleShell>;
}
