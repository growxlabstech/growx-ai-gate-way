...
export const CANONICAL_DEPENDENCIES: Record<string, ReliabilityDependency> = {
  postgres: {
    ...
    degradedBehavior: "Fail-closed on writes, bounded cache read-only if configured",
    recoveryStrategy: "Automated replica promotion / PITR point-in-time restore",
  ...
  redis: {
    ...
    recoveryStrategy: "Cold instance restart, transparent in-memory state rebuild",
    rpoMinutes: 0,
    rtoMinutes: 2,
    healthCheck: "PING",
  ...
  object_storage: {
    ...
    degradedBehavior: "Text inference unaffected; file/media uploads deferred safely",
    recoveryStrategy: "Cross-region replication failover / bucket version restore",
    rpoMinutes: 0,
    rtoMinutes: 5,
  ...
  provider_vault: {
    ...
    degradedBehavior: "Fail-closed; do not route to accounts with unresolved credentials",
  ...
  provider_upstreams: {
    ...
    degradedBehavior: "Router V2 multi-provider fallback within compatible routes",
    ...
    backupStrategy: "Provider pooling across OpenAI, Anthropic, Google, DeepSeek",
    region: "GLOBAL",
  ...
  outbox_queue: {
    ...
    recoveryStrategy: "Worker fleet scale-up and atomic batch claiming from outbox",
    rpoMinutes: 0,
    rtoMinutes: 5,
...
export class DependencyRegistry {
  ...
  constructor(initial: Record<string, ReliabilityDependency> = CANONICAL_DEPENDENCIES) {
  ...
  public listByTier(tier: string): ReliabilityDependency[] {
    return Array.from(this.dependencies.values()).filter((d) => d.criticality === tier);
  }
}
