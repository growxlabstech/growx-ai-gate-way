...
export async function createOrganization(
  ...
  return runner.transaction(async (transaction) => {
    ...
    const memberId = await transaction.createOwnerMembership(
      organizationId,
      input.userId,
    );
    await transaction.assignOwnerRole(organizationId, memberId, input.userId);
    const workspaceId = await transaction.createDefaultWorkspace(
      organizationId,
      input.userId,
      input.workspaceName,
      input.workspaceSlug,
    );
    const environmentId = await transaction.createDevelopmentEnvironment(
      organizationId,
      workspaceId,
    );
    await transaction.setDefaultEnvironment(
      organizationId,
      workspaceId,
      environmentId,
    );
    ...
    await transaction.appendOutboxEvent({
      ...
      metadata: { requestId: input.requestId, traceId: input.traceId },
    });
