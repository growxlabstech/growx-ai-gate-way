...
describe('Phase 31: Structured Output Lifecycle Integration', () => {
  ...
  describe('Upfront Schema Validation & Feature Analysis', () => {
    it('analyzes schema features and validates complexity', () => {
      ...
      expect(() => validateSupportedSubset(userProfileSchema, true)).not.toThrow();
    });
  ...
  describe('Deterministic Response Format Hashing for Cache Isolation', () => {
    it('produces identical hashes for identical schemas with different key orders', () => {
      ...
      expect(computeResponseFormatHash(rf1)).toBe(computeResponseFormatHash(rf2));
    });

    it('produces distinct hashes for different schemas', () => {
      const rf1 = { type: 'json_schema' as const, schema: { a: 1 }, strict: true };
      const rf2 = { type: 'json_schema' as const, schema: { a: 2 }, strict: true };
      expect(computeResponseFormatHash(rf1)).not.toBe(computeResponseFormatHash(rf2));
    });
  });
  ...
  describe('Local Validation Authority (GrowX validates all provider outputs)', () => {
    it('validates compliant provider output', () => {
      ...
      const parsed = parseStructuredOutput(rawProviderOutput, 'stop', { type: 'json_schema' });
      expect(parsed.failureCategory).toBeUndefined();

      const validation = validator.validateOutput(userProfileSchema, parsed.parsed);
    ...
    it('detects model refusal patterns deterministically', () => {
      const refusal = "I'm sorry, but I cannot generate personal user data as requested.";
      const parsed = parseStructuredOutput(refusal, 'stop', { type: 'json_schema' });
    ...
    it('detects truncation on finish_reason length', () => {
      ...
      const parsed = parseStructuredOutput(truncated, 'length', { type: 'json_schema' });
