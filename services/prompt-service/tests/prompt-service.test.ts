...
describe("PromptService Lifecycle & Multi-Tenancy", () => {
    ...
    const v1 = await service.createVersion(
      ...
      prompt.id,
      {
        messages: [
          {
            role: "user",
            contentTemplate: "Extract invoice details: {{raw_text}}",
          },
        ],
    ...
    const v2 = await service.createVersion(
      ...
      prompt.id,
      {
        messages: [
          {
            role: "system",
            contentTemplate: "You are an accurate accounting assistant.",
          },
          {
            role: "user",
            contentTemplate: "Extract invoice details: {{raw_text}}",
    ...
    const v2 = await service.createVersion(
      ...
      prompt.id,
      {
        messages: [
          {
            role: "user",
            contentTemplate: "Provide a 3-bullet summary of: {{text}}",
          },
        ],
        variableSchema: [{ name: "text", type: "string", required: true }],
      },
  ...
  it("enforces tenant isolation across all prompt operations", async () => {
    ...
    await expect(
      service.getPrompt("org_tenant_b", prompt.id),
