...
describe('Output Parser', () => {
  it('parses clean JSON', () => {
    const res = parseStructuredOutput('{"count": 42}', 'stop', { type: 'json_object' });
  ...
  it('strips markdown json fences safely', () => {
    ...
    const res = parseStructuredOutput(withFences, 'stop', { type: 'json_object' });
  ...
  it('detects truncation on finish_reason length', () => {
    ...
    const res = parseStructuredOutput(partial, 'length', { type: 'json_object' });
