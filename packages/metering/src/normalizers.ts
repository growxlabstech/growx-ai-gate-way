...
export function normalizeOpenAIUsage(rawUsage: any): NormalizedProviderUsage {
  ...
  const inputTokens = typeof rawUsage.prompt_tokens === "number" ? rawUsage.prompt_tokens : undefined;
  const outputTokens = typeof rawUsage.completion_tokens === "number" ? rawUsage.completion_tokens : undefined;
  const totalTokens = typeof rawUsage.total_tokens === "number" ? rawUsage.total_tokens : (inputTokens ?? 0) + (outputTokens ?? 0);
 ...
 */
export function normalizeAnthropicUsage(rawUsage: any): NormalizedProviderUsage {
  if (!rawUsage || typeof rawUsage !== "object") {
    return { source: "unavailable", confidence: "unknown" };
  }
...
export function normalizeGeminiUsage(rawUsage: any): NormalizedProviderUsage {
  ...
  const inputTokens = typeof rawUsage.promptTokenCount === "number" ? rawUsage.promptTokenCount : undefined;
  const outputTokens = typeof rawUsage.candidatesTokenCount === "number" ? rawUsage.candidatesTokenCount : undefined;
  const totalTokens = typeof rawUsage.totalTokenCount === "number" ? rawUsage.totalTokenCount : (inputTokens ?? 0) + (outputTokens ?? 0);
  const cachedInputTokens = typeof rawUsage.cachedContentTokenCount === "number" ? rawUsage.cachedContentTokenCount : undefined;
...
export function normalizeProviderUsage(
  ...
  if (rawPayload && typeof rawPayload === "object") {
    ...
    if (providerId.toLowerCase().includes("openai")) {
      ...
      if (normalized.source !== "unavailable") return normalized;
    } else if (providerId.toLowerCase().includes("gemini") || providerId.toLowerCase().includes("google")) {
      const normalized = normalizeGeminiUsage(rawUsage);
      if (normalized.source !== "unavailable") return normalized;
    } else if (typeof rawUsage.inputTokens === "number" && typeof rawUsage.outputTokens === "number") {
      return {
