...
export default async function SelectOrganizationPage() {
  ...
  if (result.status === "unauthenticated")
    redirect("/sign-in?returnTo=/select-organization");
  ...
  if (result.status === "error")
    return (
      <main className="picker">
        ...
        <p className="muted">
          GrowX could not safely load your organizations. Try again when the
  ...
  return (
    <main className="picker">
      ...
      <div className="organization-list">
        {result.context.organizations.map((organization) => {
          const workspaces = workspacesForOrganization(
            result.context,
            organization.organizationId,
          );
          const destination = workspaces[0]
            ? `/${organization.organizationSlug}/${workspaces[0].workspaceSlug}/overview`
            : `/${organization.organizationSlug}/overview`;
          return (
            <Link
